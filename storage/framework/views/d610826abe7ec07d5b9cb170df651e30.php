...
<?php $__env->startSection('page-title', 'Kartu Rencana Studi'); ?>
...
<?php $__env->startSection('content'); ?>
    <div class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p class="text-sm sm:text-base text-gray-600 dark:text-slate-400">Semester: <span
                        class="font-semibold"><?php echo e($semesterAktif->nama_semester ?? 'Tidak ada semester aktif'); ?></span> •
                    <span class="font-semibold">Semester <?php echo e($mahasiswaSemester ?? 1); ?></span></p>
                </div>
                <div class="text-left sm:text-right">
                    ...
                    <span class="inline-block px-4 py-2 rounded-lg font-bold text-sm
                        <?php if(isset($statusKrs) && $statusKrs === 'Sudah Mengisi KRS'): ?> bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400
                        <?php elseif(isset($statusKrs) && $statusKrs === 'Draft'): ?> bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400
                        <?php else: ?> bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400
                        <?php endif; ?>">
                        <?php if(isset($statusKrs) && !empty($statusKrs)): ?>
                            <?php echo e($statusKrs); ?>
            ...
            <div class="flex items-center gap-4">
                <div class="w-20 h-20 rounded-full bg-gray-100 dark:bg-slate-700 overflow-hidden flex items-center justify-center">
                    <?php if(!empty($mahasiswa->foto)): ?>
                        <img src="<?php echo e($mahasiswa->foto_url ?? ''); ?>" alt="Foto Mahasiswa" class="w-full h-full object-cover">
                    <?php else: ?>
                ...
                <div>
                    ...
                    <div class="text-lg font-bold text-gray-900 dark:text-white"><?php echo e($mahasiswa->nama ?? $mahasiswa->user->name ?? '-'); ?></div>
                    <div class="text-sm text-gray-500 dark:text-slate-400 mt-1">NIM: <span class="font-medium text-gray-700 dark:text-slate-300"><?php echo e($mahasiswa->nim ?? '-'); ?></span></div>
                </div>
            </div>
        </div>
        ...
        <?php elseif($statusKrs === 'Sudah Mengisi KRS'): ?>
        ...
        <form action="<?php echo e(route('mahasiswa.krs.store')); ?>" method="POST" id="krsForm">
            <?php echo csrf_field(); ?>
            ...
            <div class="bg-white dark:bg-[#1a1d2e] rounded-xl shadow-lg p-6 mb-6">
                ...
                <div class="flex flex-col md:flex-row gap-3">
                    <select id="mataKuliahSelect"
                        ...
                        <?php if(!$isEditable): ?> disabled aria-disabled="true" <?php endif; ?>>
                        <option value="">-- Pilih Mata Kuliah Tambahan --</option>
                        <?php $__currentLoopData = $additionalMataKuliah; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $mk): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php
                                $alreadyTaken = $existingKrs->contains('mata_kuliah_id', $mk->id);
                            ?>
                            <?php if(!$alreadyTaken): ?>
                                <option value="<?php echo e($mk->id); ?>" data-kode="<?php echo e($mk->kode_mk); ?>" data-kode-id="<?php echo e($mk->kode_id); ?>"
                                    data-nama="<?php echo e($mk->nama_mk); ?>" data-sks="<?php echo e($mk->sks); ?>" data-semester="<?php echo e($mk->semester); ?>"
                                    data-jenis="<?php echo e($mk->jenis); ?>" data-praktikum="<?php echo e($mk->praktikum ?? 0); ?>">
                                    [<?php echo e($mk->kode_id); ?>] <?php echo e($mk->kode_mk); ?> - <?php echo e($mk->nama_mk); ?> (<?php echo e($mk->sks); ?> SKS)
                                    <?php if($mk->praktikum): ?>
                    ...
                    <button type="button" id="addMataKuliahBtn"
                        ...
                        <?php if(!$isEditable): ?> disabled aria-disabled="true" class="opacity-50 cursor-not-allowed" <?php endif; ?>>
                ...
                <div class="bg-white dark:bg-[#1a1d2e] rounded-lg border border-gray-200 dark:border-slate-700 p-4 flex items-center justify-between">
                    ...
                    <div>
                        <button id="ambilSemuaBtn" type="button"
                            ...
                            <?php if(!$isEditable): ?> disabled aria-disabled="true" class="opacity-50 cursor-not-allowed"
                            <?php endif; ?>>Ambil Semua</button>
                    </div>
                </div>
            </div>
            ...
            <?php if($calendarKelas->isNotEmpty()): ?>
                    ...
                    <div class="relative w-full max-w-6xl bg-white dark:bg-[#1a1d2e] rounded-2xl md:rounded-3xl shadow-2xl flex flex-col h-[85vh] overflow-hidden transform transition-all border border-gray-100/50">
                        ...
                        <div class="flex items-center justify-between px-5 md:px-6 py-4 bg-gradient-to-r from-maroon to-[#8B1538] text-white z-20 shrink-0 border-b border-[#5C0A22]/20">
                            <div>
                                ...
                                <p class="text-[11px] md:text-sm text-white/80 mt-0.5 font-medium">Semester <?php echo e($semesterAktif->nama_semester ?? 'Aktif'); ?></p>
                            </div>
                                 ...
                                 <div id="calendarScrollContainer" class="flex-1 overflow-auto custom-scrollbar relative bg-gray-50/50 dark:bg-transparent">
                                     ...
                                     <div class="flex w-full">
                                         ...
                                         <div class="w-16 flex-shrink-0 flex flex-col pt-10 text-xs text-gray-400 font-medium text-right pr-4 select-none">
                                             <?php for($h = $hStart; $h <= $hEnd; $h++): ?>
                                                 <div class="relative" style="height: <?php echo e($rowHeight); ?>px">
                                                     <span class="absolute -top-3 right-0"><?php echo e(sprintf('%02d:00', $h)); ?></span>
                                                 </div>
                                             <?php endfor; ?>
                                         </div>
                                             ...
                                             style="display: grid; grid-template-columns: repeat(6, 1fr); gap: 1px; background-color: #f3f4f6;">
                                                 <?php $__currentLoopData = $days; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $day): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                     ...
                                                     <div class="relative bg-white dark:bg-[#1a1d2e]"
                                                         style="height: <?php echo e(($hEnd - $hStart + 1) * $rowHeight); ?>px">
                                                         ...
                                                         <div class="h-10 bg-white/90 dark:bg-[#1a1d2e]/90 border-b border-gray-100 dark:border-slate-700 flex items-center justify-center sticky top-0 z-30 backdrop-blur-md shadow-sm">
                                                             <span class="font-bold text-gray-700 dark:text-gray-300 text-[11px] uppercase tracking-wider"><?php echo e($day); ?></span>
                                                         </div>

                                                         
                                                         <div class="absolute inset-0 top-10 z-0 pointer-events-none">
                                                             <?php for($h = $hStart; $h <= $hEnd; $h++): ?>
                                                                 <div class="border-b border-gray-50 w-full" style="height: <?php echo e($rowHeight); ?>px"></div>
                                                             <?php endfor; ?>
                                                         </div>
                                                         ...
                                                         <div class="relative top-0 w-full h-full z-10">
                                                             <?php $__currentLoopData = $events; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ev): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                                 ...
                                                                 ?>
                                                                 <div class="<?php echo e($eventClass); ?> box-border absolute rounded border-l-4 p-2 <?php echo e($style['bg']); ?> <?php echo e($style['border']); ?> hover:z-50 hover:scale-[1.02] cursor-pointer shadow-sm"
                                                                     style="top: <?php echo e($top); ?>px; height: <?php echo e($duration); ?>px; left: <?php echo e($leftPercent); ?>%; width: calc(<?php echo e($widthPercent); ?>% - 2px); opacity: 0.95; z-index: 20;"
                                                                     title="<?php echo e($kelasItem->mataKuliah->nama_mk ?? ''); ?>">
                                                                     <div class="font-bold text-[11px] <?php echo e($style['text']); ?> leading-tight truncate">
                                                                         <?php echo e($kelasItem->mataKuliah->nama_mk ?? '-'); ?>

                                                                     </div>
                                                                     <div class="flex items-center gap-1 mt-1">
                                                                         <span class="text-[10px] font-bold <?php echo e($style['time']); ?>">
                                                                             <?php echo e($ev->startObj->format('H:i')); ?> - <?php echo e($ev->endObj->format('H:i')); ?>
                                 ...
                                 <div class="overflow-x-auto overflow-y-hidden">
                                     <table class="w-full text-left align-middle">
                                        ...
                                        <tbody class="divide-y divide-gray-100/80 dark:divide-slate-700/50">
                                            <?php $__empty_1 = true; $__currentLoopData = $calendarKelas->sortBy(function ($k) {
                                                    return $k->jadwals->first()->hari ?? 'Z';
                                                }); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kelas): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                                    <?php $__currentLoopData = $kelas->jadwals; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $jadwal): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                        <tr class="hover:bg-white dark:hover:bg-[#1a1d2e] transition-colors duration-200 group">
                                                            <td class="px-5 py-4">
                                                                <div class="flex flex-wrap items-center gap-2 mb-1.5">
                                                                    <div class="text-[10px] font-bold text-gray-700 dark:text-gray-300 <?php echo e(strtolower($jadwal->hari) == 'senin' ? 'text-blue-600 dark:text-blue-400' : (strtolower($jadwal->hari) == 'selasa' ? 'text-green-600 dark:text-green-400' : (strtolower($jadwal->hari) == 'rabu' ? 'text-purple-600 dark:text-purple-400' : (strtolower($jadwal->hari) == 'kamis' ? 'text-orange-600 dark:text-orange-400' : 'text-red-600 dark:text-red-400')))); ?>"><?php echo e($jadwal->hari); ?></div>
                                                                ...
                                                                <div class="font-bold text-gray-800 dark:text-gray-100 text-[13px] leading-tight group-hover:text-maroon dark:group-hover:text-rose-400 transition-colors line-clamp-2 duration-200"><?php echo e($kelas->mataKuliah->nama_mk ?? '-'); ?></div>
                                                                <div class="flex items-center gap-2 mt-1.5 line-clamp-1">
                                                                    <span class="text-[9px] bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 border border-blue-100/50 dark:border-blue-800/30 px-1.5 py-0.5 rounded font-mono font-bold"><?php echo e($kelas->mataKuliah->kode_mk ?? '-'); ?></span>
                                                                    <span class="text-[10px] text-gray-400 dark:text-gray-500 font-bold">• <?php echo e($kelas->mataKuliah->sks ?? 0); ?> SKS</span>
                                                                    <span class="text-[10px] text-gray-400 dark:text-gray-500 ml-auto truncate flex items-center gap-1">
                                                                        <i class="fas fa-user-tie text-[9px] opacity-70"></i> <?php echo e(explode(',', $kelas->dosen?->nama ?? '-')[0]); ?>

                                                                    </span>
                                                                </div>
                                                            </td>
                                                            <td class="px-3 py-4 text-center align-top pt-5">
                                                                <span class="text-[11px] font-bold text-gray-600 dark:text-gray-300"><?php echo e($jadwal->ruangan ?? '-'); ?></span>
                                                            </td>
                                                        </tr>
                                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            ...
            <div class="bg-white dark:bg-[#1a1d2e] rounded-xl shadow-lg overflow-hidden">
                ...
                <div class="bg-gradient-to-r from-maroon to-maroon-hover text-white px-6 py-4 flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xs mt-1 opacity-75">Debug: <?php echo e($availableKelas->count()); ?> kelas tersedia</p>
                        </div>
                        <div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full min-w-[1000px]" id="krsTable">
                        ...
                        <tbody class="divide-y divide-gray-200" id="krsTableBody">
                            ...
                            <?php $__currentLoopData = $currentSemesterMataKuliah; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $mk): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php
                                    $krs = $existingKrs->get($mk->id);
                                    $isChecked = $krs && $krs->ambil_mk === 'ya';
                                ?>
                                <tr class="hover:bg-gray-50 dark:hover:bg-slate-800 transition <?php echo e($isChecked ? 'bg-blue-50 dark:bg-blue-900/20' : ''); ?>" data-mk-id="<?php echo e($mk->id); ?>">
                                    <td class="px-4 py-4 text-sm text-gray-700 dark:text-slate-300"><?php echo e($loop->iteration); ?></td>
                                    <td class="px-4 py-4 text-sm font-medium text-gray-800 dark:text-white">
                                        <?php echo e($mk->kode_mk); ?>

                                        <span class="block text-xs text-gray-500 mt-1"><?php echo e(strtoupper($mk->kode_id)); ?></span>
                                    </td>
                                    ...
                                    <td class="px-4 py-4 text-sm text-center font-semibold text-gray-800"><?php echo e($mk->sks); ?></td>
                                    <td class="px-4 py-4 text-sm text-center text-gray-700"><?php echo e($mk->semester); ?></td>
                                    <td class="px-4 py-4 text-center">
                                        ...
                                        ?>
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold <?php echo e($colorClass); ?>">
                                    ...
                                    <td class="px-4 py-4 text-sm text-gray-700 text-center">
                                        <?php echo e($kelas?->dosen?->nama ?? '-'); ?>

                                    </td>

                                    
                                    <td class="px-4 py-4 text-sm text-center text-gray-600">
                                        <?php if($kelas && $kelas->jadwals && $kelas->jadwals->count()): ?>
                                            <?php $jadwal = $kelas->jadwals->first(); ?>
                                            <div class="text-xs leading-tight">
                                                <div><?php echo e($jadwal->hari); ?></div>
                                                <div><?php echo e(substr($jadwal->jam_mulai, 0, 5)); ?> - <?php echo e(substr($jadwal->jam_selesai, 0, 5)); ?></div>
                                                <div class="font-semibold"><?php echo e($jadwal->ruangan); ?></div>
                                            </div>
                                        <?php else: ?>
                                            -
                                        <?php endif; ?>
                            ...
                            <?php $__currentLoopData = $existingKrs->where('ambil_mk', 'ya'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $krs): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php if($krs->mataKuliah && $krs->mataKuliah->kode_id !== $currentKodeId): ?>
                                    <?php $mk = $krs->mataKuliah; ?>
                                    <tr class="hover:bg-gray-50 transition bg-green-50" data-mk-id="<?php echo e($mk->id); ?>" data-additional="true">
                                        <td class="px-4 py-4 text-sm text-gray-700"><?php echo e($currentSemesterMataKuliah->count() + $loop->iteration); ?></td>
                                        <td class="px-4 py-4 text-sm font-medium text-gray-800">
                                            <?php echo e($mk->kode_mk); ?>

                                            <span class="block text-xs text-green-600 mt-1"><?php echo e(strtoupper($mk->kode_id)); ?> • Lintas Semester</span>
                                        </td>
                                        <td class="px-4 py-4 text-sm text-gray-800">
                                            <?php echo e($mk->nama_mk); ?>
                                        ...
                                        <td class="px-4 py-4 text-center">
                                            <button type="button" class="remove-additional-btn px-3 py-1 bg-red-500 text-white text-xs rounded hover:bg-red-600 transition" data-mk-id="<?php echo e($mk->id); ?>" <?php if(!$isEditable): ?> disabled aria-disabled="true" <?php endif; ?>>
                                                <i class="fas fa-trash"></i> Hapus
                                            </button>
                                            <input type="hidden" name="mata_kuliah[<?php echo e($mk->id); ?>]" value="ya">
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>

                            <?php if($currentSemesterMataKuliah->isEmpty() && $existingKrs->where('ambil_mk', 'ya')->isEmpty()): ?>
                                <tr id="emptyRow">
                                    <td colspan="<?php echo e($isEditable ? '9' : '8'); ?>" class="px-4 py-8 text-center text-gray-500 dark:text-slate-500">
                ...
                <div class="bg-gray-50 dark:bg-slate-800 px-4 sm:px-6 py-5 border-t-2 border-gray-200 dark:border-slate-700">
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                        ...
                        <div class="flex flex-col sm:flex-row gap-3">
                            <button type="submit" name="action" value="draft" class="w-full sm:w-auto px-4 sm:px-6 py-3 font-bold rounded-lg transition shadow-md hover:shadow-lg flex items-center justify-center gap-2 text-white border <?php echo e(!$isEditable ? 'bg-maroon border-maroon cursor-not-allowed opacity-50' : 'bg-maroon border-maroon hover:bg-red-800'); ?>" <?php if(!$isEditable): ?> disabled aria-disabled="true" <?php endif; ?>>
                                <i class="fas <?php echo e(!$isEditable ? 'fa-lock' : 'fa-save'); ?>"></i>
                                <span class="text-sm sm:text-base">Simpan Draft</span>
                            </button>
                            <button type="submit" name="action" value="submit" class="w-full sm:w-auto px-4 sm:px-6 py-3 font-bold rounded-lg transition shadow-md hover:shadow-lg flex items-center justify-center gap-2 text-white <?php echo e(!$isEditable ? 'bg-maroon cursor-not-allowed opacity-50' : 'bg-maroon hover:bg-red-800'); ?>" id="submitBtn" <?php if(!$isEditable): ?> disabled aria-disabled="true" <?php endif; ?>>
                                <i class="fas <?php echo e(!$isEditable ? 'fa-lock' : 'fa-paper-plane'); ?>"></i>
...
<?php echo $__env->make('layouts.mahasiswa', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/naradata/STIH-SIAKAD/resources/views/page/mahasiswa/krs/index.blade.php ENDPATH**/ ?>