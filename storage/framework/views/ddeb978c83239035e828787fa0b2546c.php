<?php $__env->startSection('title', 'Kelas Mata Kuliah'); ?>
<?php $__env->startSection('page-title', 'Kelas Mata Kuliah'); ?>
<?php $__env->startSection('content'); ?>
    ...
    <div x-data="kelasMatKuliahData()"
        x-init="$watch('selectedMeeting', () => { if (this.showDetail) this.loadAttendanceData(); })">
        <div class="mb-6 flex items-start justify-between">
            <div>
                <h3 class="text-2xl font-bold text-gray-800 flex items-center"><i
                        class="fas fa-chalkboard-teacher text-maroon mr-2"></i>Daftar Kelas Mata Kuliah</h3>
                <p class="text-sm text-gray-600 mt-1">Kelola pengelompokan kelas per mata kuliah (Hanya menampilkan kelas
                    dengan jadwal aktif atau yang memiliki jadwal sendiri)</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg  overflow-hidden">
                ...
                <table class="w-full">
                    <thead class="bg-maroon text-white">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-sm font-semibold">Kapasitas</th>
                            <th class="px-6 py-4 text-left text-sm font-semibold">Ruangan</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        <?php $__empty_1 = true; $__currentLoopData = $kelasMatKul; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $k): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-blue-50 transition duration-200 cursor-pointer"
                                :class="{'bg-blue-50': selectedKelasId === <?php echo e($k->id); ?> && showDetail}"
                                @click="toggleDetail(<?php echo e($k->id); ?>)">
                                <td class="px-6 py-4">
                                    <div class="font-semibold text-gray-900"><?php echo e($k->mataKuliah->nama_mk); ?></div>
                                    <div class="text-sm text-gray-500"><?php echo e($k->mataKuliah->kode_mk); ?></div>
                                </td>
                                <td class="px-6 py-4"><span
                                        class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800"><i
                                            class="fas fa-users mr-1"></i><?php echo e($k->nama_kelas); ?></span></td>
                                <td class="px-6 py-4"><?php echo e($k->dosen->user->name); ?></td>
                                <td class="px-6 py-4">
                                    <?php echo e($k->jadwal?->kelas?->semester_type ?? $k->semester?->nama_semester ?? '-'); ?></td>
                                <td class="px-6 py-4">
                                    <?php if($k->jadwal): ?>
                                        <div class="text-sm">
                                            <div class="font-semibold text-gray-900"><?php echo e($k->jadwal->hari); ?></div>
                                            <div class="text-xs text-gray-500"><?php echo e(substr($k->jadwal->jam_mulai, 0, 5)); ?> -
                                                <?php echo e(substr($k->jadwal->jam_selesai, 0, 5)); ?></div>
                                        </div>
                                    <?php elseif($k->hari && $k->jam_mulai && $k->jam_selesai): ?>
                                        <div class="text-sm">
                                            <div class="font-semibold text-gray-900"><?php echo e($k->hari); ?></div>
                                            <div class="text-xs text-gray-500"><?php echo e(substr($k->jam_mulai, 0, 5)); ?> -
                                                <?php echo e(substr($k->jam_selesai, 0, 5)); ?></div>
                                        </div>
                                    <?php else: ?>
                                ...
                                <td class="px-6 py-4"><span
                                        class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-gray-100 text-gray-800"><?php echo e($k->kapasitas); ?></span>
                                </td>
                                <td class="px-6 py-4"><?php echo e($k->ruang ?: '-'); ?></td>
                            </tr>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center text-gray-500"><i
                                        class="fas fa-inbox text-4xl mb-3"></i>
                                    <p class="text-lg font-semibold">Belum ada kelas mata kuliah dengan jadwal aktif</p>
                                    <p class="text-sm mt-2">Kelas akan ditampilkan setelah memiliki jadwal aktif atau jam kuliah
                                        tersedia</p>
                                </td>
                            </tr>
                        <?php endif; ?>
            ...
            <?php if($kelasMatKul->hasPages()): ?>
                <div class="px-6 py-4 border-t border-gray-200 bg-gray-50"><?php echo e($kelasMatKul->links()); ?></div>
            <?php endif; ?>
        </div>

        
        <div x-ref="detailCard" x-show="showDetail" x-transition class="mt-8 bg-white rounded-xl shadow-lg  overflow-hidden"
            style="display: none;">
            <div class="p-6 border-b border-gray-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h3 class="text-xl font-bold text-gray-800 flex items-center">
                        ...
                        <template x-if="attendanceData?.pertemuan?.tipe === 'uts'">
                            <span
                                class="ml-2 px-2 py-0.5 text-xs font-bold bg-amber-100 text-amber-800 rounded-full">UTS</span>
                        </template>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <select id="meetingSelect" x-model="selectedMeeting" @change="loadAttendanceData()"
                        class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-maroon focus:border-transparent text-sm min-w-[280px]">
            ...
            <div class="px-6 py-4 grid grid-cols-1 md:grid-cols-4 gap-6 border-b border-gray-200 bg-gray-50">
                <div>
                    ...
                    <p class="font-bold text-gray-800 flex items-center gap-2">
                        ...
                        <span x-show="!loadingData && attendanceData?.pertemuan?.tanggal"
                            x-text="attendanceData?.pertemuan?.tanggal ? new Date(attendanceData.pertemuan.tanggal).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : '-'"></span>
                ...
                <div>
                    ...
                    <p class="font-bold text-gray-800 flex items-center gap-2">
                        ...
                        <span x-show="!loadingData && attendanceData?.jadwal"
                            x-text="`${attendanceData?.jadwal?.jam_mulai || '-'} - ${attendanceData?.jadwal?.jam_selesai || '-'} WIB`"></span>
                ...
                <div class="flex items-center gap-4 mb-3">
                    <div class="flex-1">
                        <label class="block text-sm font-bold text-gray-800 mb-2">
                            <i class="fas fa-broadcast-tower text-blue-600 mr-2"></i>Link Zoom Umum (Berlaku Untuk Semua
                            Pertemuan)
                        </label>
                        <div class="flex gap-3">
                            <input type="url" x-model="generalMeetingLink" placeholder="https://zoom.us/j/..."
                            ...
                            <button @click="saveGeneralMeetingLink()" x-bind:disabled="savingGeneralLink"
                        ...
                        <p class="text-xs text-blue-700 mt-2">Link ini akan otomatis digunakan untuk semua pertemuan. Namun
                            setiap pertemuan tetap bisa memiliki link custom sendiri.</p>
                    </div>
                </div>
            </div>

            
            <div x-show="showLinksTable && attendanceData?.pertemuan" x-transition
                ...
                <div class="flex items-center gap-4 mb-3">
                    <div class="flex-1">
                        ...
                        <p class="text-xs text-gray-600">Kosongkan untuk menggunakan link umum, atau isi untuk menggunakan
                            link custom untuk pertemuan ini</p>
                    </div>
                </div>
                        ...
                        <table class="w-full text-sm">
                            ...
                            <tbody>
                                <tr class="border-b border-gray-200 hover:bg-white transition">
                                    <td class="px-4 py-3">
                                        <div class="font-semibold text-gray-900" x-text="attendanceData.pertemuan.label">
                                        </div>
                                    </td>
                                    <td class="px-4 py-3 text-gray-600" x-text="attendanceData.pertemuan.tanggal || '-'">
                                    </td>
                                    ...
                                    <td class="px-4 py-3">
                                        ...
                                        <template x-if="editingLinkId !== attendanceData.pertemuan.id">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <template
                                                    x-if="!attendanceData.pertemuan.online_meeting_link && generalMeetingLink">
                ...
                <table class="w-full">
                    <thead class="bg-gray-50 text-gray-700">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-center text-xs font-bold uppercase tracking-wider">Status Kehadiran
                            </th>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <!-- No Data State -->
                        <template
                            x-if="!loadingData && (!attendanceData || !attendanceData.students || attendanceData.students.length === 0)">
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                                    <div class="flex flex-col items-center justify-center">
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-3 text-center">
                                        <span :class="{
                                            ...
                                            x-text="student.status.toUpperCase()"></span>
                                        <template
                                            x-if="student.status === 'hadir' && student.distance_meters !== null && student.distance_meters !== undefined">
                                            <div class="text-[10px] text-gray-500 mt-0.5"
                                                x-text="'(' + Math.round(student.distance_meters) + 'm dari kampus)'"></div>
                                        </template>
                                    </td>
                                    <td class="px-6 py-3 text-center text-sm text-gray-600" x-text="student.waktu_scan">
                                    </td>
                                </tr>
                            </template>
                        </template>
            ...
            <div class="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-between items-center">
                <div class="text-xs text-gray-500">
                    <span x-show="!loadingData"
                        x-text="`Menampilkan ${attendanceData?.students?.length || 0} dari ${attendanceData?.total_students || 0} mahasiswa`"></span>
                    <span x-show="loadingData">Memuat...</span>
                    <span x-show="!loadingData && attendanceData && typeof attendanceData.total_hadir !== 'undefined'"
                        class="ml-4 font-semibold text-green-600"
                        x-text="`Hadir: ${attendanceData?.total_hadir || 0}`"></span>
                    <span x-show="!loadingData && attendanceData && typeof attendanceData.total_tidak_hadir !== 'undefined'"
                        class="ml-4 font-semibold text-red-600"
                        x-text="`Tidak Hadir: ${attendanceData?.total_tidak_hadir || 0}`"></span>
                </div>
            </div>
        </div>
    </div>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/naradata/STIH-SIAKAD/resources/views/admin/kelas-mata-kuliah/index.blade.php ENDPATH**/ ?>