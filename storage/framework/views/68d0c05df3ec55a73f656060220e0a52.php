<?php $__env->startSection('title', 'Pengajuan Mahasiswa'); ?>
...
<?php $__env->startSection('content'); ?>
    <div class="px-4 sm:px-6 lg:px-8 pt-2 pb-8 w-full max-w-9xl mx-auto font-inter">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div class="relative overflow-hidden bg-white dark:bg-bg-card rounded-2xl p-6 shadow-sm border border-border-color group hover:border-blue-500/30 transition-colors">
                <div class="relative z-10">
                    ...
                    <div class="text-3xl font-bold text-text-primary group-hover:scale-105 transition-transform origin-left"><?php echo e($allPengajuans->count()); ?></div>
            ...
            <div class="relative overflow-hidden bg-white dark:bg-bg-card rounded-2xl p-6 shadow-sm border border-border-color group hover:border-yellow-500/30 transition-colors">
                <div class="relative z-10">
                    ...
                    <div class="text-3xl font-bold text-text-primary group-hover:scale-105 transition-transform origin-left"><?php echo e($allPengajuans->where('status', 'submitted')->count()); ?></div>
            ...
            <div class="relative overflow-hidden bg-white dark:bg-bg-card rounded-2xl p-6 shadow-sm border border-border-color group hover:border-green-500/30 transition-colors">
                <div class="relative z-10">
                    ...
                    <div class="text-3xl font-bold text-text-primary group-hover:scale-105 transition-transform origin-left"><?php echo e($allPengajuans->where('status', 'approved')->count()); ?></div>
        ...
        <div class="bg-white dark:bg-bg-card border border-border-color rounded-2xl shadow-sm overflow-hidden">
            <header class="px-6 py-5 border-b border-border-color flex items-center justify-between bg-gray-50/30 dark:bg-transparent">
                ...
                <div class="text-xs text-text-muted">Menampilkan <?php echo e($pengajuans->count()); ?> data terakhir</div>
            </header>
                            ...
                            <tr class="hover:bg-gray-50/60 dark:hover:bg-bg-hover/40 transition-colors group">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        ...
                                        ?>
                                        <div class="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 <?php echo e($bg); ?>">
                                            <i class="fas <?php echo e($icon); ?> text-sm"></i>
                                        </div>
                                        <span class="font-medium text-text-primary text-sm"><?php echo e($p->jenis_label); ?></span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-text-secondary font-medium"><?php echo e($p->created_at->format('d M Y')); ?></div>
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    <?php if($p->file_path): ?>
                                        <a href="<?php echo e(Storage::url($p->file_path)); ?>" target="_blank" class="inline-flex items-center gap-1 mt-1 text-xs text-primary hover:underline">
                                ...
                                <td class="px-6 py-4">
                                    <?php if($p->rejected_reason): ?>
                                        <div class="text-sm text-red-700 italic bg-red-50 rounded-md px-3 py-2 border border-red-200">
                                            <i class="fas fa-exclamation-circle mr-1"></i><?php echo e($p->rejected_reason); ?>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-1.5 flex-wrap">
                                        ...
                                        <?php if($p->status === 'draft'): ?>
                                            <button onclick="openPengajuanWizard({id: <?php echo e($p->id); ?>, jenis: '<?php echo e($p->jenis); ?>', step: 2, isRevision: false})"
                                            ...
                                            <button onclick="deletePengajuan(<?php echo e($p->id); ?>)"
                                        ...
                                        <?php elseif($p->status === 'generated'): ?>
                                            <a href="<?php echo e(route('mahasiswa.pengajuan.download-generated', $p->id)); ?>"
                                            ...
                                            <button onclick="openPengajuanWizard({id: <?php echo e($p->id); ?>, jenis: '<?php echo e($p->jenis); ?>', step: 4, isRevision: false})"
                                        ...
                                        <?php elseif($p->status === 'rejected'): ?>
                                            <button onclick="openPengajuanWizard({id: <?php echo e($p->id); ?>, jenis: '<?php echo e($p->jenis); ?>', step: 4, isRevision: true})"
                                        ...
                                        <?php elseif($p->status === 'approved' && $p->file_surat): ?>
                                            <button @click="$dispatch('open-preview-modal', '<?php echo e(route('mahasiswa.pengajuan.preview', $p->id)); ?>')"
                                                class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-xs font-medium transition-colors">
                                                <i class="fas fa-eye"></i> Lihat
                                            </button>
                                            <a href="<?php echo e(route('mahasiswa.pengajuan.download', $p->id)); ?>"
    ...
    <script>
        window.__pengajuanJenisConfig = <?php echo json_encode($jenisConfig, 15, 512) ?>;
        window.__mahasiswa = <?php echo json_encode(['semester' => optional(auth()->user()->mahasiswa)->semester ?? null], 15, 512) ?>;
        window.__mahasiswa_courses = <?php echo json_encode($currentCourses ?? [], 15, 512) ?>;
    </script>
            ...
            <div
                ...
                <div x-show="step === 3" class="px-6 py-5">
                    ...
                    <div x-show="!generating" class="space-y-4">
                        ...
                        <a :href="`<?php echo e(url('mahasiswa/pengajuan')); ?>/${pengajuanId}/download-generated`"
                           target="_blank"
...
function pengajuanWizard(jenisConfig) {
    return {
        ...
        async submitDraft() {
            ...
            try {
                const resp = await fetch('<?php echo e(route("mahasiswa.pengajuan.store")); ?>', {
                ...
                // Dispatch generate
                await fetch(`<?php echo e(url('mahasiswa/pengajuan')); ?>/${this.pengajuanId}/generate`, {
                    method: 'POST',
        ...
        async pollStatus() {
            ...
            const check = async () => {
                ...
                attempt++;
                try {
                    const resp = await fetch(`<?php echo e(url('mahasiswa/pengajuan')); ?>/${this.pengajuanId}/status`);
                    const data = await resp.json();
        ...
        async doUploadSigned() {
            ...
            xhr.open('POST', `<?php echo e(url('mahasiswa/pengajuan')); ?>/${this.pengajuanId}/upload-signed`);
        ...
        async doSubmit() {
            ...
            this.errorMsg = '';
            try {
                const resp = await fetch(`<?php echo e(url('mahasiswa/pengajuan')); ?>/${this.pengajuanId}/submit`, {
                    method: 'POST',
                    headers: {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.deletePengajuan = async (id) => {
        ...
        try {
            const resp = await fetch(`<?php echo e(url('mahasiswa/pengajuan')); ?>/${id}`, {
                method: 'DELETE',
                headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' },
            });
            const data = await resp.json();
...
<?php echo $__env->make('layouts.mahasiswa', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/naradata/STIH-SIAKAD/resources/views/page/mahasiswa/pengajuan/index.blade.php ENDPATH**/ ?>