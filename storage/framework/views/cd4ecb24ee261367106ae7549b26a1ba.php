...
<?php $__env->startSection('page-title', 'Manajemen Mata Kuliah'); ?>
    ...
    <div x-data="mkPage()" x-init="init()">
        ...
        <div class="mb-5 flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
            <div>
                ...
                <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">Master MK global &middot; Relasi per Tahun Ajaran
                    &middot; Histori &amp; Carry-Forward</p>

                
                <div class="mt-2 flex flex-wrap items-center gap-2">
                    <?php if($activeSemester): ?>
                        <span
                            class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-semibold bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300">
                            <span class="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
                            Semester Aktif: <?php echo e($activeSemester->display_label); ?>

                        </span>
                        <?php if($activeSemester->is_locked): ?>
                            <span
                                class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400">
                                <i class="fas fa-lock"></i> DIKUNCI
                            </span>
                        <?php endif; ?>
                    <?php else: ?>
                        <span
                            class="px-3 py-1 rounded-full text-sm font-semibold bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300">
            ...
            <div class="flex flex-wrap gap-2 flex-shrink-0 items-start">
                ...
                <a href="<?php echo e(route('admin.mata-kuliah.create')); ?>"
        ...
        <?php if(session('success')): ?>
            <div x-data="{ show: true }" x-show="show" x-transition
                ...
                <button @click="show = false" class="ml-auto text-green-600 hover:text-green-800 flex-shrink-0"><i
        ...
        <?php if(session('error')): ?>
            <div x-data="{ show: true }" x-show="show" x-transition
                ...
                <button @click="show = false" class="ml-auto text-red-600 hover:text-red-800 flex-shrink-0"><i
                        class="fas fa-times"></i></button>
            </div>
        <?php endif; ?>
        ...
        <div
            class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-200 dark:border-gray-700 px-4 py-3 mb-3 flex flex-wrap gap-3 items-center">

            <form method="GET" action="<?php echo e(route('admin.mata-kuliah.index')); ?>" class="flex gap-2 items-center">
                <input type="hidden" name="tab" value="<?php echo e(in_array($tab, ['ta-aktif', 'histori']) ? $tab : 'ta-aktif'); ?>">
                <label class="text-xs font-semibold text-gray-500 dark:text-gray-400 whitespace-nowrap"><i
                        class="fas fa-calendar-alt mr-1"></i>Semester:</label>
                <select name="semester_id" onchange="this.form.submit()"
                    ...
                    <?php $__currentLoopData = $allSemesters; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sem): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($sem->id); ?>" <?php echo e($selectedSemester?->id == $sem->id ? 'selected' : ''); ?>>
                            <?php echo e($sem->display_label); ?><?php echo e($sem->is_active ? ' (Aktif)' : ''); ?>
            ...
            <?php if($selectedSemester && !$selectedSemester->is_locked): ?>
            ...
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-show="open" x-transition
                    ...
                    <?php $__currentLoopData = $allSemesters; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sem): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <form action="<?php echo e(route('admin.mata-kuliah-semester.activate-semester', $sem)); ?>" method="POST"
                            onsubmit="return confirm('Aktifkan semester <?php echo e($sem->display_label); ?>?')">
                            <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                            <button type="submit"
                                class="w-full text-left px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center gap-2 <?php echo e($sem->is_active ? 'font-bold text-green-700 dark:text-green-400' : 'text-gray-700 dark:text-gray-300'); ?>">
                                <?php if($sem->is_active): ?><i class="fas fa-check-circle text-green-500 w-4"></i><?php else: ?><span
                                class="w-4 inline-block"></span><?php endif; ?>
                                <?php echo e($sem->display_label); ?>
            ...
            <?php if($selectedSemester): ?>
                <?php if($selectedSemester->is_locked): ?>
                    <form action="<?php echo e(route('admin.mata-kuliah-semester.unlock-semester', $selectedSemester)); ?>" method="POST"
                        onsubmit="return confirm('Buka kunci semester <?php echo e($selectedSemester->display_label); ?>?')">
                        <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                        <button type="submit"
                            class="flex items-center gap-1.5 px-3 py-1.5 bg-orange-500 text-white rounded-lg hover:bg-orange-600 text-xs font-semibold transition">
                            <i class="fas fa-lock-open"></i> Buka Kunci
                        </button>
                    </form>
                <?php else: ?>
                    <form action="<?php echo e(route('admin.mata-kuliah-semester.lock-semester', $selectedSemester)); ?>" method="POST"
                        onsubmit="return confirm('Kunci semester <?php echo e($selectedSemester->display_label); ?>?')">
                        <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                        <button type="submit"
                            class="flex items-center gap-1.5 px-3 py-1.5 bg-gray-700 text-white rounded-lg hover:bg-gray-800 text-xs font-semibold transition">
            ...
            <div class="ml-auto flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
                <?php if($selectedSemester): ?>
                    <span>MK Aktif TA: <strong
                            class="text-maroon dark:text-red-400"><?php echo e($activePivots->count()); ?></strong></span>
                    <span class="text-gray-300 dark:text-gray-600">|</span>
                    <span>Histori: <strong><?php echo e($historyPivots->count()); ?></strong></span>
                    <?php if($selectedSemester->is_locked): ?>
                        <span
                            class="px-2 py-0.5 rounded-full bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 font-bold">
        ...
        <div
            class="flex items-center border border-b-0 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 rounded-t-xl overflow-visible">
            <a href="<?php echo e(route('admin.mata-kuliah.index', ['tab' => 'master', 'semester_id' => $selectedSemester?->id])); ?>"
                class="px-5 py-3 text-sm font-semibold border-b-2 transition flex-shrink-0 <?php echo e($tab === 'master' ? 'border-maroon text-maroon dark:text-red-400 dark:border-red-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-maroon'); ?>">
                <i class="fas fa-database mr-1.5"></i> Master MK
                <span
                    class="ml-1 px-1.5 py-0.5 rounded-full text-xs <?php echo e($tab === 'master' ? 'bg-maroon text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400'); ?>"><?php echo e($mataKuliahs->total()); ?></span>
            </a>
            <a href="<?php echo e(route('admin.mata-kuliah.index', ['tab' => 'ta-aktif', 'semester_id' => $selectedSemester?->id])); ?>"
                class="px-5 py-3 text-sm font-semibold border-b-2 transition flex-shrink-0 <?php echo e($tab === 'ta-aktif' ? 'border-maroon text-maroon dark:text-red-400 dark:border-red-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-maroon'); ?>">
                <i class="fas fa-check-circle mr-1.5"></i> TA Aktif
                <span
                    class="ml-1 px-1.5 py-0.5 rounded-full text-xs <?php echo e($tab === 'ta-aktif' ? 'bg-maroon text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400'); ?>"><?php echo e($activePivots->count()); ?></span>
            </a>
            <a href="<?php echo e(route('admin.mata-kuliah.index', ['tab' => 'histori', 'semester_id' => $selectedSemester?->id])); ?>"
                class="px-5 py-3 text-sm font-semibold border-b-2 transition flex-shrink-0 <?php echo e($tab === 'histori' ? 'border-maroon text-maroon dark:text-red-400 dark:border-red-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-maroon'); ?>">
                <i class="fas fa-archive mr-1.5"></i> Histori TA
                <span
                    class="ml-1 px-1.5 py-0.5 rounded-full text-xs <?php echo e($tab === 'histori' ? 'bg-maroon text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400'); ?>"><?php echo e($historyPivots->count()); ?></span>
            </a>
            ...
            <div class="ml-auto flex items-center gap-2 px-3 py-2">
                <?php if($tab === 'master'): ?>
                    ...
                    <form method="GET" action="<?php echo e(route('admin.mata-kuliah.index')); ?>" class="flex items-center gap-2"
                        id="master-filter-form">
                        <input type="hidden" name="tab" value="master">
                        <input type="hidden" name="semester_id" value="<?php echo e($selectedSemester?->id); ?>">
                        <div class="relative">
                            <i class="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
                            <input type="text" id="mk-search" name="search" value="<?php echo e($search); ?>"
                                placeholder="Cari kode / nama MK..."
                                class="pl-9 pr-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-maroon w-52"
                                onkeydown="if(event.key==='Enter'){this.form.submit();}">
                        </div>
                        <select name="semester_filter" id="mk-semester-filter" onchange="this.form.submit()"
                            class="text-sm border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg px-3 py-1.5 focus:ring-2 focus:ring-maroon">
                            <option value="">Semua Semester</option>
                            <?php for($s = 1; $s <= 8; $s++): ?>
                                <option value="<?php echo e($s); ?>" <?php echo e($semesterFilter == $s ? 'selected' : ''); ?>>Semester <?php echo e($s); ?></option>
                            <?php endfor; ?>
                        </select>
                        <?php if($search || $semesterFilter): ?>
                            <a href="<?php echo e(route('admin.mata-kuliah.index', ['tab' => 'master', 'semester_id' => $selectedSemester?->id])); ?>"
                                class="px-3 py-1.5 text-sm text-gray-600 dark:text-gray-400 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 transition">
                ...
                <?php else: ?>
                    ...
                    <div class="relative">
                        <i class="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
                        <input type="text" id="mk-search" placeholder="Cari kode / nama MK..." oninput="filterCurrentTab()"
                            class="pl-9 pr-3 py-1.5 text-sm border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-maroon w-52">
                    </div>
                    <select id="mk-semester-filter" onchange="filterCurrentTab()"
                        class="text-sm border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg px-3 py-1.5 focus:ring-2 focus:ring-maroon">
                        <option value="">Semua Semester</option>
                        <?php for($s = 1; $s <= 8; $s++): ?>
                    ...
                    <button type="button" id="mk-reset-btn" onclick="resetMkFilter()"
                        class="hidden px-3 py-1.5 text-sm text-gray-600 dark:text-gray-400 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 transition">
                        <i class="fas fa-times"></i>
                    </button>
                <?php endif; ?>
            </div>
        ...
        <div
            class="bg-white dark:bg-gray-800 rounded-b-xl shadow border border-t-0 border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <?php if($tab === 'master'): ?>
                            ...
                            <tr>
                                ...
                                <?php
                                    ...
                                    $sortIcon = function ($col) use ($sort, $sortDir) {
                                        if ($sort !== $col)
                                            return '<i class="fas fa-sort ml-1 opacity-40"></i>';
                                        return $sortDir === 'asc' ? '<i class="fas fa-sort-up ml-1"></i>' : '<i class="fas fa-sort-down ml-1"></i>';
                                    };
                                ?>
                                <th class="px-5 py-4 text-left text-xs font-bold uppercase"><a href="<?php echo e($sortUrl('kode_mk')); ?>"
                                        class="hover:underline">Kode MK <?php echo $sortIcon('kode_mk'); ?></a></th>
                                <th class="px-5 py-4 text-left text-xs font-bold uppercase"><a href="<?php echo e($sortUrl('nama_mk')); ?>"
                                        class="hover:underline"><i class="fas fa-book-open mr-1"></i>Nama Mata Kuliah
                                        <?php echo $sortIcon('nama_mk'); ?></a></th>
                                <th class="px-5 py-4 text-center text-xs font-bold uppercase"><a href="<?php echo e($sortUrl('sks')); ?>"
                                        class="hover:underline"><i class="fas fa-calculator mr-1"></i>SKS
                                        <?php echo $sortIcon('sks'); ?></a></th>
                                <th class="px-5 py-4 text-center text-xs font-bold uppercase"><a href="<?php echo e($sortUrl('jenis')); ?>"
                                        class="hover:underline"><i class="fas fa-tags mr-1"></i>Jenis
                                        <?php echo $sortIcon('jenis'); ?></a></th>
                                <th class="px-5 py-4 text-left text-xs font-bold uppercase"><i
                                        class="fas fa-graduation-cap mr-1"></i>Prodi</th>
                                <th class="px-5 py-4 text-center text-xs font-bold uppercase"><i
                                        class="fas fa-cog mr-1"></i>Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700" id="master-tbody">
                            <?php $__empty_1 = true; $__currentLoopData = $mataKuliahs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $mk): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition master-row"
                                            data-kode="<?php echo e(strtolower($mk->kode_mk)); ?>" data-nama="<?php echo e(strtolower($mk->nama_mk)); ?>"
                                            data-semester="<?php echo e($mk->semester); ?>" data-sks="<?php echo e($mk->sks); ?>">
                                            <td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-400 font-medium master-row-no">
                                                <?php echo e(($mataKuliahs->currentPage() - 1) * $mataKuliahs->perPage() + $loop->iteration); ?>

                                            </td>
                                            <td class="px-5 py-4">
                                                <span
                                                    class="font-mono text-sm text-maroon dark:text-red-400 font-bold"><?php echo e($mk->kode_mk); ?></span>
                                            </td>
                                            <td class="px-5 py-4">
                                                <div class="flex items-center gap-3">
                                                    <div
                                                        class="h-9 w-9 rounded-lg bg-maroon flex items-center justify-center text-white flex-shrink-0">
                                                        <i class="fas fa-book text-xs"></i>
                                                    </div>
                                                    <div>
                                                        <div class="text-sm font-semibold text-gray-900 dark:text-gray-100">
                                                            <?php echo e($mk->nama_mk); ?></div>
                                                        <div class="text-xs text-gray-500 dark:text-gray-400">Semester <?php echo e($mk->semester); ?>
                                            ...
                                            <td class="px-5 py-4 text-center">
                                                <span
                                                    class="px-2.5 py-1 text-sm font-bold rounded-full bg-red-100 dark:bg-red-900/30 text-maroon dark:text-red-300">
                                            ...
                                            <td class="px-5 py-4 text-center">
                                                ...
                                                ?>
                                                <span class="px-2.5 py-1 text-xs font-semibold rounded-full <?php echo e($colorClass); ?>">
                                                    <i class="fas fa-star mr-1"></i><?php echo e(ucwords(str_replace('_', ' ', $mk->jenis))); ?>
                                            ...
                                            <td class="px-5 py-4">
                                                ...
                                                <?php echo e($mk->prodi->jenjang === 'S1' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300'
                                : ($mk->prodi->jenjang === 'S2' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300'
                                    : 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300')); ?>">
                                                        <?php echo e($mk->prodi->jenjang); ?>

                                                    </span>
                                                    <div>
                                                        <div class="text-sm font-medium text-gray-900 dark:text-gray-100">
                                                            <?php echo e($mk->prodi->nama_prodi); ?></div>
                                                        <div class="text-xs text-gray-500 dark:text-gray-400">
                                                            <?php echo e($mk->fakultas->nama_fakultas); ?></div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-5 py-4 text-center">
                                        ...
                                        <div id="modal-mk-<?php echo e($mk->id); ?>"
                                            class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm hidden">
                                            <div
                                                class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-11/12 md:w-3/4 lg:w-1/2 max-h-[90vh] overflow-y-auto">
                                                <div class="flex items-center justify-between px-6 py-4 bg-maroon text-white rounded-t-xl">
                                                    <div class="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            ...
                                                            <p class="text-xs text-white/80"><?php echo e($mk->kode_mk); ?></p>
                                                        </div>
                                                    </div>
                                                    <button
                                                        onclick="document.getElementById('modal-mk-<?php echo e($mk->id); ?>').classList.add('hidden')"
                                                ...
                                                <div class="p-6 grid grid-cols-2 gap-4 text-sm">
                                                    <div class="col-span-2">
                                                        ...
                                                        <p class="font-semibold text-gray-900 dark:text-gray-100"><?php echo e($mk->nama_mk); ?></p>
                                                    </div>
                                                    <div>
                                                        <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">Kode MK</p>
                                                        <p class="font-mono font-bold text-maroon dark:text-red-400"><?php echo e($mk->kode_mk); ?></p>
                                                    </div>
                                                    <div>
                                                        <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">SKS</p>
                                                        <p class="font-semibold text-gray-900 dark:text-gray-100"><?php echo e($mk->sks); ?></p>
                                                    </div>
                                                    <div>
                                                        <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">Semester</p>
                                                        <p class="font-semibold text-gray-900 dark:text-gray-100"><?php echo e($mk->semester); ?></p>
                                                    </div>
                                                    <div>
                                                        <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">Jenis</p>
                                                        <p class="font-semibold text-gray-900 dark:text-gray-100">
                                                            <?php echo e(ucwords(str_replace('_', ' ', $mk->jenis))); ?></p>
                                                    </div>
                                                    <div>
                                                        <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">Prodi</p>
                                                        <p class="font-semibold text-gray-900 dark:text-gray-100">
                                                            <?php echo e($mk->prodi->nama_prodi); ?></p>
                                                    </div>
                                                    <div>
                                                        <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">Fakultas</p>
                                                        <p class="font-semibold text-gray-900 dark:text-gray-100">
                                                            <?php echo e($mk->fakultas->nama_fakultas); ?></p>
                                                    </div>
                                                </div>
                                                <div class="px-6 py-4 bg-gray-50 dark:bg-gray-700 flex justify-end gap-3 rounded-b-xl">
                                                    <a href="<?php echo e(route('admin.mata-kuliah.edit', $mk)); ?>"
                                                        class="bg-maroon text-white px-4 py-2 rounded-lg text-sm shadow">Edit</a>
                                                    <button
                                                        onclick="document.getElementById('modal-mk-<?php echo e($mk->id); ?>').classList.add('hidden')"
                ...
                <?php if($mataKuliahs->hasPages()): ?>
                    <div class="bg-gray-50 dark:bg-gray-800 px-6 py-4 border-t border-gray-200 dark:border-gray-700">
                        <?php echo e($mataKuliahs->links(null, ['path' => route('admin.mata-kuliah.index'), 'query' => array_filter(['tab' => 'master', 'semester_id' => $selectedSemester?->id, 'search' => $search, 'semester_filter' => $semesterFilter, 'sort' => $sort, 'sort_dir' => $sortDir], fn($v) => $v !== '' && $v !== null)])); ?>

                    </div>
                <?php endif; ?>

                
            <?php elseif($tab === 'ta-aktif'): ?>
                <?php if($activePivots->count() > 0): ?>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700" id="active-table">
                            <thead class="bg-maroon text-white">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-bold uppercase cursor-pointer select-none"
                                        onclick="sortActiveTable('kode_mk')">Kode MK <i class="fas fa-sort ml-1 opacity-40"
                                            id="active-sort-icon-kode_mk"></i></th>
                                    <th class="px-4 py-3 text-left text-xs font-bold uppercase cursor-pointer select-none"
                                        onclick="sortActiveTable('nama_mk')">Nama Mata Kuliah <i class="fas fa-sort ml-1 opacity-40"
                                            id="active-sort-icon-nama_mk"></i></th>
                                    <th class="px-4 py-3 text-center text-xs font-bold uppercase cursor-pointer select-none"
                                        onclick="sortActiveTable('sks')">SKS <i class="fas fa-sort ml-1 opacity-40"
                                            id="active-sort-icon-sks"></i></th>
                                    <th class="px-4 py-3 text-left text-xs font-bold uppercase">Prodi</th>
                                    <th class="px-4 py-3 text-left text-xs font-bold uppercase cursor-pointer select-none"
                                        onclick="sortActiveTable('semester')">Semester <i class="fas fa-sort ml-1 opacity-40"
                                            id="active-sort-icon-semester"></i></th>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700" id="active-tbody">
                                <?php $__currentLoopData = $activePivots; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $pivot): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <?php $mk = $pivot->mataKuliah; ?>
                                    <tr class="hover:bg-red-50 dark:hover:bg-red-900/10 transition active-row"
                                        data-kode="<?php echo e(strtolower($mk->kode_mk)); ?>" data-nama="<?php echo e(strtolower($mk->nama_mk)); ?>"
                                        data-semester="<?php echo e($mk->semester); ?>" data-sks="<?php echo e($mk->sks); ?>">
                                        <td class="px-4 py-3">
                                            <input type="checkbox" value="<?php echo e($mk->id); ?>"
                                                class="active-mk-checkbox w-4 h-4 rounded border-gray-300 text-maroon">
                                        </td>
                                        <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400 active-row-no"><?php echo e($i + 1); ?></td>
                                        <td class="px-4 py-3">
                                            <span
                                                class="font-mono text-xs font-bold text-maroon dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2 py-0.5 rounded"><?php echo e($mk->kode_mk); ?></span>
                                        </td>
                                        <td class="px-4 py-3">
                                            <div class="text-sm font-semibold text-gray-900 dark:text-gray-100"><?php echo e($mk->nama_mk); ?></div>
                                        </td>
                                        <td class="px-4 py-3 text-center">
                                            <span
                                                class="px-2 py-0.5 text-xs font-bold rounded-full bg-red-100 dark:bg-red-900/30 text-maroon dark:text-red-300"><?php echo e($mk->sks); ?>

                                                SKS</span>
                                        </td>
                                        <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300"><?php echo e($mk->prodi?->nama_prodi); ?>

                                        </td>
                                        <td class="px-4 py-3">
                                            <span
                                                class="px-2 py-0.5 text-xs font-semibold rounded-full bg-red-100 dark:bg-red-900/30 text-maroon dark:text-red-300">Semester
                                                <?php echo e($mk->semester); ?></span>
                                        </td>
                                        <td class="px-4 py-3">
                                            <?php if($pivot->source_semester_id): ?>
                                                <span
                                                    class="inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400">
                                                    <i class="fas fa-copy text-xs"></i> <?php echo e($pivot->sourceSemester?->display_label); ?>
                                        ...
                                        <td class="px-4 py-3 text-center text-xs text-gray-500 dark:text-gray-400">
                                            <?php echo e($pivot->activated_at ? $pivot->activated_at->format('d/m/Y') : '-'); ?>

                                        </td>
                                        <td class="px-4 py-3 text-center">
                                            <?php if($selectedSemester && !$selectedSemester->is_locked): ?>
                                                <form method="POST" action="<?php echo e(route('admin.mata-kuliah-semester.detach')); ?>"
                                                    onsubmit="return confirm('Pindah ke histori?')">
                                                    <?php echo csrf_field(); ?>
                                                    <input type="hidden" name="semester_id" value="<?php echo e($selectedSemester->id); ?>">
                                                    <input type="hidden" name="mata_kuliah_ids[]" value="<?php echo e($mk->id); ?>">
                                                    <button type="submit"
                ...
                <?php else: ?>
                    <div class="text-center py-16 text-gray-400 dark:text-gray-500">
                        ...
                        <p class="text-sm mt-1">Gunakan <strong>Tambah ke Semester</strong> atau <strong>Carry Forward</strong> di
                            atas.</p>
                    </div>
                <?php endif; ?>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700" id="history-table">
                            <thead class="bg-maroon text-white">
                                <tr>
                                    <th class="px-4 py-3 text-left text-xs font-bold uppercase">No</th>
                                    <th class="px-4 py-3 text-left text-xs font-bold uppercase cursor-pointer select-none"
                                        onclick="sortHistoryTable('kode_mk')">Kode MK <i class="fas fa-sort ml-1 opacity-40"
                                            id="history-sort-icon-kode_mk"></i></th>
                                    <th class="px-4 py-3 text-left text-xs font-bold uppercase cursor-pointer select-none"
                                        onclick="sortHistoryTable('nama_mk')">Nama Mata Kuliah <i
                                            class="fas fa-sort ml-1 opacity-40" id="history-sort-icon-nama_mk"></i></th>
                                    <th class="px-4 py-3 text-center text-xs font-bold uppercase cursor-pointer select-none"
                                        onclick="sortHistoryTable('sks')">SKS <i class="fas fa-sort ml-1 opacity-40"
                                            id="history-sort-icon-sks"></i></th>
                                    <th class="px-4 py-3 text-center text-xs font-bold uppercase cursor-pointer select-none"
                                        onclick="sortHistoryTable('semester')">Semester <i class="fas fa-sort ml-1 opacity-40"
                                            id="history-sort-icon-semester"></i></th>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700" id="history-tbody">
                                <?php $__currentLoopData = $historyPivots; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $pivot): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <?php $mk = $pivot->mataKuliah; ?>
                                    <tr class="hover:bg-red-50 dark:hover:bg-red-900/10 transition history-row <?php echo e($pivot->status === 'archived' ? 'opacity-60' : ''); ?>"
                                        data-kode="<?php echo e(strtolower($mk->kode_mk)); ?>" data-nama="<?php echo e(strtolower($mk->nama_mk)); ?>"
                                        data-semester="<?php echo e($mk->semester); ?>" data-sks="<?php echo e($mk->sks); ?>">
                                        <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400 history-row-no"><?php echo e($i + 1); ?></td>
                                        <td class="px-4 py-3">
                                            <span
                                                class="font-mono text-xs font-bold text-maroon dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2 py-0.5 rounded"><?php echo e($mk->kode_mk); ?></span>
                                        </td>
                                        <td class="px-4 py-3">
                                            <div class="text-sm font-semibold text-gray-900 dark:text-gray-100"><?php echo e($mk->nama_mk); ?></div>
                                            <div class="text-xs text-gray-500 dark:text-gray-400"><?php echo e($mk->sks); ?> SKS &bull; Semester
                                                <?php echo e($mk->semester); ?></div>
                                        </td>
                                        <td class="px-4 py-3 text-center">
                                            <span
                                                class="px-2 py-0.5 text-xs font-bold rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400"><?php echo e($mk->sks); ?>
                                        ...
                                        <td class="px-4 py-3 text-center">
                                            <?php if($pivot->status === 'history'): ?>
                                                <span
                                                    class="inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded-full bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400">
                                                    <i class="fas fa-clock text-xs"></i> Histori
                                                </span>
                                            <?php else: ?>
                                                <span
                                                    class="inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded-full bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-400">
                                                    <i class="fas fa-archive text-xs"></i> Arsip
                                                </span>
                                            <?php endif; ?>
                                        </td>
                                        <td class="px-4 py-3 text-xs text-gray-500 dark:text-gray-400">
                                            <?php echo e($pivot->sourceSemester?->display_label ?? '—'); ?></td>
                                        <td class="px-4 py-3 text-center text-xs text-gray-500 dark:text-gray-400">
                                            <?php echo e($pivot->deactivated_at ? $pivot->deactivated_at->format('d/m/Y') : '-'); ?>

                                        </td>
                                        <td class="px-4 py-3 text-center">
                                            <?php if($activeSemester && !$activeSemester->is_locked): ?>
                                                <form method="POST" action="<?php echo e(route('admin.mata-kuliah-semester.restore')); ?>"
                                                    onsubmit="return confirm('Pulihkan MK ini ke semester aktif?')">
                                                    <?php echo csrf_field(); ?>
                                                    <input type="hidden" name="source_semester_id" value="<?php echo e($selectedSemester->id); ?>">
                                                    <input type="hidden" name="target_semester_id" value="<?php echo e($activeSemester->id); ?>">
                                                    <input type="hidden" name="mata_kuliah_ids[]" value="<?php echo e($mk->id); ?>">
                                                    <button type="submit"
        ...
        <div x-show="attachOpen" x-cloak
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col"
                ...
                <div class="flex items-center justify-between px-6 py-4 bg-maroon text-white rounded-t-2xl">
                    <h3 class="text-lg font-bold flex items-center gap-2"><i class="fas fa-plus-circle"></i> Tambah MK ke
                        Semester</h3>
                    <button @click="attachOpen = false" class="text-white text-xl">&times;</button>
                </div>
                <form method="POST" action="<?php echo e(route('admin.mata-kuliah-semester.attach')); ?>"
                    class="flex flex-col flex-1 overflow-hidden">
                    <?php echo csrf_field(); ?>
                    <input type="hidden" name="semester_id" value="<?php echo e($selectedSemester?->id); ?>">
                    <div class="p-5 flex-1 overflow-y-auto">
                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
                            Pilih mata kuliah dari master untuk ditambahkan ke semester
                            <strong><?php echo e($selectedSemester?->display_label); ?></strong>.
                        </p>
                        ...
                        <div id="attachList"
                            class="border dark:border-gray-700 rounded-xl divide-y divide-gray-100 dark:divide-gray-700 max-h-80 overflow-y-auto">
                            <?php $__empty_1 = true; $__currentLoopData = $unattachedMK; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $mk): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <label
                                    class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer attach-item"
                                    data-search="<?php echo e(strtolower($mk->kode_mk . ' ' . $mk->nama_mk)); ?>">
                                    <input type="checkbox" name="mata_kuliah_ids[]" value="<?php echo e($mk->id); ?>"
                                        class="w-4 h-4 text-maroon border-gray-300 rounded">
                                    <div class="flex-1 min-w-0">
                                        <div class="text-sm font-semibold text-gray-800 dark:text-gray-100"><?php echo e($mk->nama_mk); ?>

                                        </div>
                                        <div class="text-xs text-gray-500 dark:text-gray-400">
                                            <?php echo e($mk->kode_mk); ?> &bull; <?php echo e($mk->sks); ?> SKS &bull; <?php echo e($mk->prodi?->nama_prodi); ?>

                                        </div>
                                    </div>
                                    <span
                                        class="px-2 py-0.5 text-xs rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 flex-shrink-0">Semester
                                        <?php echo e($mk->semester); ?></span>
                                </label>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                        ...
                        <div class="mt-2 flex gap-3 text-xs">
                            <button type="button"
                                onclick="document.querySelectorAll('#attachList input[type=checkbox]').forEach(c=>c.checked=true)"
                                class="text-maroon dark:text-red-400 hover:underline">Pilih Semua</button>
                            <span class="text-gray-300">|</span>
                            <button type="button"
                                onclick="document.querySelectorAll('#attachList input[type=checkbox]').forEach(c=>c.checked=false)"
                                class="text-gray-500 hover:underline">Batal Pilih</button>
                        </div>
                    </div>
                    <div
                        class="px-6 py-4 bg-gray-50 dark:bg-gray-700/50 border-t dark:border-gray-700 flex justify-end gap-3 rounded-b-2xl">
                        <button type="button" @click="attachOpen = false"
                            class="px-5 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-700 dark:text-gray-300">Batal</button>
                        <button type="submit"
                            class="px-5 py-2 bg-maroon text-white rounded-lg text-sm font-semibold hover:bg-red-900">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                ...
                <div class="p-5 flex-1 overflow-y-auto">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1">Semester
                                Sumber</label>
                            <select id="cf-source" onchange="cfPreview()"
                                ...
                                <?php $__currentLoopData = $allSemesters; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sem): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($sem->id); ?>"><?php echo e($sem->display_label); ?></option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                        </div>
                        <div>
                            ...
                            <select id="cf-target" onchange="cfPreview()"
                                ...
                                <?php $__currentLoopData = $allSemesters; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sem): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($sem->id); ?>" <?php echo e($selectedSemester?->id == $sem->id ? 'selected' : ''); ?>>
                                        <?php echo e($sem->display_label); ?></option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div id="cf-preview" class="hidden space-y-3">
                        ...
                        <div id="cf-to-copy-section" class="hidden">
                            <p class="text-xs font-semibold text-green-700 dark:text-green-400 mb-1"><i
                                    class="fas fa-check mr-1"></i> Akan disalin:</p>
                            <div id="cf-to-copy"
                                class="border dark:border-gray-700 rounded-lg divide-y divide-gray-100 dark:divide-gray-700 max-h-44 overflow-y-auto text-xs">
                            </div>
                        </div>
                        <div id="cf-conflict-section" class="hidden">
                            <p class="text-xs font-semibold text-orange-600 dark:text-orange-400 mb-1"><i
                                    class="fas fa-exclamation-triangle mr-1"></i> Konflik (dilewati):</p>
                            <div id="cf-conflicts"
                                class="border dark:border-gray-700 rounded-lg divide-y divide-gray-100 dark:divide-gray-700 max-h-28 overflow-y-auto text-xs">
                            </div>
                        </div>
                    </div>
                </div>
                <form id="cf-form" method="POST" action="<?php echo e(route('admin.mata-kuliah-semester.carry-forward')); ?>">
                    <?php echo csrf_field(); ?>
                    <input type="hidden" id="cf-form-source" name="source_semester_id">
                    <input type="hidden" id="cf-form-target" name="target_semester_id">
                    <div
                        class="px-6 py-4 bg-gray-50 dark:bg-gray-700/50 border-t dark:border-gray-700 flex justify-end gap-3 rounded-b-2xl">
                        <button type="button" @click="carryOpen = false"
                            class="px-5 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-700 dark:text-gray-300">Batal</button>
        ...
        <div x-show="restoreOpen" x-cloak
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                <div class="flex items-center justify-between px-6 py-4 bg-maroon text-white rounded-t-2xl">
                    <h3 class="text-lg font-bold flex items-center gap-2"><i class="fas fa-history"></i> Ambil dari Histori
                    </h3>
                ...
                <form method="POST" action="<?php echo e(route('admin.mata-kuliah-semester.restore')); ?>"
                    ...
                    <div class="p-5 flex-1 overflow-y-auto">
                        <div class="mb-4">
                            <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400 mb-1">Pilih Semester
                                Histori Sumber</label>
                            <select name="source_semester_id"
                                class="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg px-3 py-2 text-sm">
                                <option value="">-- Pilih Semester --</option>
                                <?php $__currentLoopData = $allSemesters; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sem): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($sem->id); ?>" <?php echo e($sem->id === $selectedSemester?->id ? 'selected' : ''); ?>>
                                        <?php echo e($sem->display_label); ?></option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                        </div>
                        ...
                        <div id="restore-list"
                            class="border dark:border-gray-700 rounded-xl divide-y divide-gray-100 dark:divide-gray-700 max-h-72 overflow-y-auto">
                            <?php $__empty_1 = true; $__currentLoopData = $historyPivots; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pivot): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <label
                                    class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer restore-item"
                                    data-search="<?php echo e(strtolower($pivot->mataKuliah->kode_mk . ' ' . $pivot->mataKuliah->nama_mk)); ?>">
                                    <input type="checkbox" name="mata_kuliah_ids[]" value="<?php echo e($pivot->mata_kuliah_id); ?>"
                                        class="w-4 h-4 text-maroon border-gray-300 rounded">
                                    <div class="flex-1 min-w-0">
                                        <div class="text-sm font-semibold text-gray-800 dark:text-gray-100">
                                            <?php echo e($pivot->mataKuliah->nama_mk); ?></div>
                                        <div class="text-xs text-gray-500 dark:text-gray-400">
                                            <?php echo e($pivot->mataKuliah->kode_mk); ?> &bull; <?php echo e($pivot->mataKuliah->sks); ?> SKS
                                        </div>
                                    </div>
                                    <span
                                        class="px-2 py-0.5 text-xs rounded-full bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400"><?php echo e($pivot->status); ?></span>
                                </label>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                                <div class="text-center py-8 text-gray-400 dark:text-gray-500">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[92vh] flex flex-col">
                ...
                <div class="flex-1 min-h-0 overflow-y-auto p-6 space-y-5" style="-webkit-overflow-scrolling: touch;">
                    ...
                    <div class="flex items-center gap-3 flex-wrap">
                        ...
                        <a href="<?php echo e(route('admin.import.template', ['type' => 'mata_kuliah', 'format' => 'csv'])); ?>"
                            data-no-loader
                        ...
                        <a href="<?php echo e(route('admin.import.template', ['type' => 'mata_kuliah', 'format' => 'xlsx'])); ?>"
                            data-no-loader
                    ...
                    <div
                        class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-900/30 rounded-xl p-4">
                        ...
                        <div class="flex flex-wrap gap-2 text-xs">
                            <?php $__currentLoopData = ['kode_mk', 'nama_matkul', 'sks', 'semester']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $col): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <span
                                    class="px-2 py-1 rounded-full bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 font-semibold"><?php echo e($col); ?>

                                    *</span>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            <?php $__currentLoopData = ['kode_id', 'jenis', 'praktikum', 'prodi_id', 'fakultas_id', 'deskripsi']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $col): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <span
                                    class="px-2 py-1 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400"><?php echo e($col); ?></span>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        </div>
                        <p class="text-xs text-blue-700 dark:text-blue-400 mt-2"><span
                                class="text-red-600 font-bold">*</span> wajib &bull; <code
                                class="bg-blue-100 dark:bg-blue-900/40 px-1 rounded">jenis</code> otomatis dari prefix
                            kode_mk (ADH1=nasional, ADH2=prodi, ADH3=pilihan, ADH4=peminatan)</p>
                    </div>
                    ...
                    <div id="imp-dropzone"
                        ...
                        ondragleave="this.classList.remove('border-maroon','bg-maroon/5')" ondrop="handleImportDrop(event)">
                        <input type="file" id="imp-file-input" class="hidden" accept=".csv,.xlsx,.xls"
                            onchange="handleImportFileSelect(this.files[0])">
                        <div id="imp-dz-idle">
                            <i class="fas fa-cloud-upload-alt text-4xl text-gray-300 dark:text-gray-600 mb-3"></i>
                            <p class="text-gray-600 dark:text-gray-400 font-medium">Drag &amp; drop file di sini atau klik
                                untuk memilih</p>
                            <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">Format: CSV, XLSX &bull; Maksimal: 10MB
                            </p>
                        </div>
                        <div id="imp-dz-selected" class="hidden">
                            <div class="flex items-center justify-center gap-3">
                                ...
                                <button type="button" onclick="event.stopPropagation();clearImportFile()"
                                    class="ml-3 text-red-400 hover:text-red-600 text-xl"><i
                    ...
                    <div class="space-y-3">
                        <label class="flex items-center gap-3 cursor-pointer">
                            ...
                            <span class="text-sm text-gray-700 dark:text-gray-300">
                                Lewati data duplikat <span class="text-gray-400 dark:text-gray-500">(kode_mk yang sudah ada
                                    tidak diperbarui)</span>
                            </span>
                        </label>
                        ...
                        <div id="imp-semester-select-wrap" class="hidden pl-8">
                            <label class="block text-xs text-gray-500 dark:text-gray-400 mb-1">Pilih semester
                                tujuan:</label>
                            <select id="imp-semester-id"
                                ...
                                <?php $__currentLoopData = $allSemesters; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sem): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($sem->id); ?>" <?php echo e(($activeSemester && $sem->id === $activeSemester->id) ? 'selected' : ''); ?>>
                                        <?php echo e($sem->display_label); ?><?php echo e($sem->status === 'active' ? ' (Aktif)' : ($sem->status === 'locked' ? ' 🔒' : '')); ?>
                    ...
                    <div id="imp-progress" class="hidden">
                        ...
                        <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 overflow-hidden">
                            <div id="imp-progress-bar" class="bg-maroon h-2 rounded-full transition-all duration-300"
                                style="width:0%"></div>
                        </div>
                    </div>

                    
                    <div id="imp-val-errors"
                        ...
                        <ul id="imp-val-error-list"
                            class="text-xs text-red-700 dark:text-red-400 list-disc list-inside space-y-1 max-h-36 overflow-y-auto">
                        </ul>
                    </div>
                    ...
                    <div id="imp-preview" class="hidden">
                        ...
                        <div class="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
                            ...
                            <div id="imp-preview-pagination"
                                class="px-3 py-2 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/30 hidden">
                                <div class="flex items-center justify-center gap-2 text-xs"
                                    id="imp-preview-pagination-inner"></div>
                            </div>
                            <p id="imp-preview-more"
                                class="hidden text-xs text-gray-400 dark:text-gray-500 text-center p-2 border-t border-gray-100 dark:border-gray-700">
                            </p>
                        </div>
                    </div>
                    ...
                    <div id="imp-result" class="hidden rounded-xl p-5 border">
                        ...
                        <div id="imp-result-detail"
                            class="text-xs text-red-700 dark:text-red-400 space-y-1 max-h-32 overflow-y-auto"></div>
                    </div>

                    
                    <div id="imp-attach-result"
                        class="hidden rounded-xl p-4 border bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800">
                ...
                <div
                    class="flex-shrink-0 border-t border-gray-200 dark:border-gray-700 px-6 py-4 flex items-center justify-between gap-3">
                    <p class="text-xs text-gray-400 dark:text-gray-500">Import ke Master MK &bull; opsional: tambahkan ke TA
                        Aktif</p>
    ...
    <?php $__env->startPush('scripts'); ?>
        <script>
            ...
            function _filterTable(rows, searchEl, semesterEl, countEl, noClass, resetBtnEl) {
                ...
                if (el) el.textContent = isFiltered ? `Menampilkan ${visible} dari ${rows.length} MK` : '';

                const resetBtn = document.getElementById(resetBtnEl);
            ...
            // Unified filter using shared inputs
            const _activeTab = '<?php echo e($tab); ?>';
            const _tabMap = {
                'master': { rows: '.master-row', noClass: 'master-row-no' },
                'ta-aktif': { rows: '.active-row', noClass: 'active-row-no' },
                'histori': { rows: '.history-row', noClass: 'history-row-no' },
            };

            function filterCurrentTab() {
                const cfg = _tabMap[_activeTab];
                if (!cfg) return;
                // Master tab uses server-side pagination, don't renumber
                if (_activeTab === 'master') return;
                const q = (document.getElementById('mk-search')?.value || '').toLowerCase();
                const sem = document.getElementById('mk-semester-filter')?.value || '';
                const resetBtn = document.getElementById('mk-reset-btn');
                if (resetBtn) resetBtn.classList.toggle('hidden', !q && !sem);
                let num = 0;
                document.querySelectorAll(cfg.rows).forEach(row => {
                    const kode = (row.dataset.kode || '').toLowerCase();
                    const nama = (row.dataset.nama || '').toLowerCase();
                    const rowSem = row.dataset.semester || '';
                    const show = (kode.includes(q) || nama.includes(q)) && (!sem || rowSem === sem);
                    row.style.display = show ? '' : 'none';
                    if (show) { num++; const noEl = row.querySelector('.' + cfg.noClass); if (noEl) noEl.textContent = num; }
                });
            }
            function resetMkFilter() {
                const s = document.getElementById('mk-search'); if (s) s.value = '';
                const f = document.getElementById('mk-semester-filter'); if (f) f.value = '';
                filterCurrentTab();
            }

            // Keep old names as aliases (used by sort callbacks)
            function filterMasterTable() { filterCurrentTab(); }
            function filterActiveTable() { filterCurrentTab(); }
            function filterHistoryTable() { filterCurrentTab(); }

            function sortActiveTable(col) { _sortTable('active-tbody', 'active-row', col, 'active'); filterCurrentTab(); }
            function sortHistoryTable(col) { _sortTable('history-tbody', 'history-row', col, 'history'); filterCurrentTab(); }
            ...
            async function runImportPreview() {
                ...
                try {
                    ...
                    const res = await fetch('<?php echo e(route('admin.import.preview', ['type' => 'mata_kuliah'])); ?>', {
                        method: 'POST', body: fd,
            ...
            async function runImport() {
                ...
                try {
                    ...
                    const res = await fetch('<?php echo e(route('admin.import.import', ['type' => 'mata_kuliah'])); ?>', {
                        method: 'POST', body: fd,
                        headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' }
                    });
                    const json = await res.json();
            ...
            async function _attachImportedToSemester(semesterId, kodeMKs) {
                ...
                try {
                    const res = await fetch('<?php echo e(route('admin.mata-kuliah-semester.attach-by-codes')); ?>', {
                        method: 'POST', body: fd,
                        headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' }
                    });
                    const json = await res.json();
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/naradata/STIH-SIAKAD/resources/views/admin/mata-kuliah/index.blade.php ENDPATH**/ ?>