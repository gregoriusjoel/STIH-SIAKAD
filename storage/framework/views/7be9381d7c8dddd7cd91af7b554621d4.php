<div class="space-y-8">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700/50 p-4">
        <div class="flex flex-col sm:flex-row sm:items-center gap-3 sm:justify-between">
            ...
            <form action="<?php echo e(route('mahasiswa.nilai.print.rangkuman')); ?>" method="GET" target="_blank" data-no-loader class="flex items-center gap-2">
                <select name="semester"
                    ...
                    <?php $__currentLoopData = ($semesterOptions ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $opt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    ...
    <?php if($nilaiPerSemester->isEmpty()): ?>
    ...
    <?php else: ?>
        <?php $__currentLoopData = $nilaiPerSemester; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $semesterNama => $nilaiList): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <?php
                $totalSKS = 0;
                $semesterNumber = null;
            ...
            <div class="bg-white dark:bg-slate-800 rounded-[32px] shadow-sm border border-slate-100 dark:border-slate-700/50 overflow-hidden">
                ...
                <div class="bg-primary px-8 py-5 border-b border-primary/5">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-5">
                            ...
                            <div>
                                <h4 class="text-xl font-black text-white"><?php echo e($semesterLabel); ?></h4>
                                <div class="flex items-center gap-3 mt-1">
                                    <span class="text-[10px] font-black bg-white/20 text-white px-2 py-1 rounded-md uppercase tracking-wider"><?php echo e($nilaiList->count()); ?> Mata Kuliah</span>
                                    <span class="size-1 bg-white/30 rounded-full"></span>
                                    <span class="text-[10px] font-black bg-white/10 text-white/80 px-2 py-1 rounded-md uppercase tracking-wider"><?php echo e($totalSKS); ?> SKS</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="p-4 md:p-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-start">
                        <?php $__currentLoopData = $nilaiList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $krs): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <?php
                                $mataKuliah = $krs->kelasMataKuliah->mataKuliah ?? null;
                                $nilai = $krs->nilai;
                            ...
                            <div class="bg-white dark:bg-slate-900/50 rounded-[24px] border border-slate-100 dark:border-slate-700/50 hover:shadow-xl hover:shadow-primary/5 transition-all duration-300 group flex flex-col cursor-pointer select-none"
                                        ...
                                        <div class="flex items-center justify-between">
                                            <div class="flex items-center gap-3">
                                                <div>
                                                    ...
                                                    <p class="text-2xl font-black text-slate-900 dark:text-white tracking-tighter leading-none"><?php echo e(number_format($nilaiAngka, 0)); ?></p>
                                                </div>
                                                <div class="h-8 w-px bg-slate-100 dark:bg-slate-700"></div>
                                                <div class="<?php echo e($gradeStyle); ?> border px-3 py-1.5 rounded-lg flex items-center gap-2">
                                                    <span class="text-[10px] font-black uppercase tracking-widest opacity-60">Grade</span>
                                                    <span class="text-sm font-black leading-none"><?php echo e($grade); ?></span>
                                                </div>
                                            </div>
                                            <div class="text-right">
                                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5">SKS</p>
                                                <p class="text-sm font-black text-slate-900 dark:text-white leading-none"><?php echo e($mataKuliah->sks ?? 0); ?></p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <?php if($bobotPenilaian || $krs->is_internship_conversion): ?>
                                    <div x-show="expanded" x-collapse>
                                        <div class="px-5 pb-5 pt-1 border-t border-dashed border-slate-100 dark:border-slate-800">
                                            <p class="text-[9px] font-black text-primary uppercase tracking-[0.15em] mb-3 flex items-center gap-2">
                                                ...
                                                Komponen
                                                <?php if($krs->is_internship_conversion && !$bobotPenilaian): ?>
                                                    ...
                                                    <?php if($comp['bobot'] > 0): ?>
                                                        <div class="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-2.5 border border-slate-100 dark:border-slate-800 flex flex-col justify-between">
                                                            <div class="flex items-center justify-between mb-1">
                                                                <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none"><?php echo e($comp['label']); ?></p>
                                                                <span class="text-[9px] font-bold text-primary bg-primary/5 px-1 py-0.5 rounded leading-none"><?php echo e(number_format($comp['bobot'], 2)); ?>%</span>
                                                            </div>
                                                            <div class="flex items-baseline leading-none mt-1">
                                                                <span class="text-lg font-black text-slate-900 dark:text-white"><?php echo e(number_format($comp['nilai'], 0)); ?></span>
                                                            </div>
                                                        </div>
                                                    <?php endif; ?>
                                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                            ...
                                            <div class="mt-3 flex items-center justify-between p-2.5 bg-slate-900 dark:bg-primary rounded-xl text-white">
                                                ...
                                                <span class="text-sm font-black"><?php echo e(number_format($nilai->bobot ?? 0, 2)); ?></span>
                                            </div>
                                        </div>
                                    </div>
                                <?php endif; ?>
                ...
                <div class="bg-slate-50 dark:bg-slate-900/50 px-8 py-4 border-t border-slate-100 dark:border-slate-800 flex flex-wrap gap-6 items-center">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm font-black text-primary"><?php echo e(number_format($ipsPerSemester[$semesterNama]['ips'] ?? 0, 2)); ?></span>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm font-black text-slate-700 dark:text-slate-300"><?php echo e($ipsPerSemester[$semesterNama]['sks'] ?? 0); ?> SKS</span>
                    </div>
                </div>
            </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
</style><?php /**PATH /Users/naradata/STIH-SIAKAD/resources/views/page/mahasiswa/nilai/_content.blade.php ENDPATH**/ ?>