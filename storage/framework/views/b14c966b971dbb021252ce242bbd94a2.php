<?php $__env->startSection('page-title', 'Manajemen Tagihan'); ?>
...
<div class="space-y-8 animate-fade-in px-4 md:px-0">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <?php $__currentLoopData = $statsItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-all group overflow-hidden relative">
                ...
                <div class="relative z-10 flex items-center gap-4">
                    <div class="size-12 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-[#8B1538] group-hover:text-white transition-all duration-300">
                        <i class="fas <?php echo e($item['icon']); ?> text-xl"></i>
                    </div>
                    <div>
                        <p class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-0.5"><?php echo e($item['label']); ?></p>
                        <h3 class="text-2xl font-black text-slate-800 leading-none"><?php echo e(number_format($item['value'])); ?></h3>
                    </div>
                </div>
            </div>
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div class="relative" x-data="{ open: false }">
                    <button @click="open = !open" 
                        ...
                        ?>
                        <span><?php echo e($selectedLabel); ?></span>
                    ...
                    <div x-show="open" 
                        ...
                        <?php $__currentLoopData = $filterOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $opt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <a href="<?php echo e(route('finance.invoices.index', ['status' => $opt['status']])); ?>" 
                               class="flex items-center justify-between px-4 py-2.5 text-sm font-bold <?php echo e(request('status') == $opt['status'] ? 'text-[#8B1538] bg-[#8B1538]/5' : 'text-slate-600 hover:bg-slate-50'); ?> transition-colors">
                                <span><?php echo e($opt['label']); ?></span>
                                <?php if(request('status') == $opt['status']): ?>
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $invoices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $invoice): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="group hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-8 py-6">
                                <div class="flex items-center gap-3">
                                    <div class="size-10 rounded-xl bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center text-slate-500 font-bold shrink-0">
                                        <?php echo e(substr($invoice->student->user->name ?? 'N', 0, 1)); ?>

                                    </div>
                                    <div class="min-w-0">
                                        <p class="text-sm font-bold text-slate-800 truncate"><?php echo e($invoice->student->user->name ?? 'N/A'); ?></p>
                                        <p class="text-xs text-slate-400 font-medium tracking-tight"><?php echo e($invoice->student->nim ?? 'N/A'); ?></p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-8 py-6">
                                <div class="inline-flex flex-col">
                                    <span class="text-sm font-bold text-slate-700">Semester <?php echo e($invoice->semester); ?></span>
                                    <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest"><?php echo e($invoice->tahun_ajaran); ?></span>
                                </div>
                            </td>
                            <td class="px-8 py-6 text-right">
                                <span class="text-sm font-black text-slate-900 tracking-tight">Rp <?php echo e(number_format($invoice->total_tagihan, 0, ',', '.')); ?></span>
                            </td>
                            <td class="px-8 py-6 text-center">
                                ...
                                ?>
                                <span class="px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest border <?php echo e($class); ?>">
                                    <?php echo e($label); ?>
...
<?php echo $__env->make('layouts.finance', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/naradata/STIH-SIAKAD/resources/views/finance/invoices/index.blade.php ENDPATH**/ ?>