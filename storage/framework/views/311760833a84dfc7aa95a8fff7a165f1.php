<?php $__env->startSection('page-title', 'Detail Bukti Pembayaran'); ?>

<?php $__env->startSection('content'); ?>
<div class="px-4 md:px-0 animate-fade-in pb-20">
    
    <div class="mb-8 flex items-center justify-between">
        <a href="<?php echo e(route('finance.payment-proofs.index')); ?>" 
        ...
        <span class="px-4 py-1.5 rounded-full border border-amber-100 bg-amber-50 text-amber-600 text-[10px] font-black uppercase tracking-widest shadow-sm">
            <?php echo e($paymentProof->status); ?>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-4 space-y-8">
            ...
            <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8">
                <div class="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <h2 class="text-lg font-black text-slate-800 leading-tight tracking-tight"><?php echo e($paymentProof->invoice->student->user->name ?? 'N/A'); ?></h2>
                        <p class="text-[10px] font-black uppercase text-slate-400 tracking-widest mt-0.5"><?php echo e($paymentProof->invoice->student->nim ?? 'N/A'); ?></p>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    <div class="p-6 bg-[#8B1538] rounded-[2rem] text-white shadow-lg shadow-red-900/10">
                        ...
                        <p class="text-3xl font-black tracking-tighter">Rp <?php echo e(number_format($paymentProof->amount_submitted, 0, ',', '.')); ?></p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <div class="p-4 bg-slate-50 border border-slate-100/50 rounded-2xl">
                            ...
                            <p class="text-sm font-black text-slate-700"><?php echo e($paymentProof->method ?? '-'); ?></p>
                        </div>
                        <div class="p-4 bg-slate-50 border border-slate-100/50 rounded-2xl">
                            <p class="text-[9px] font-black uppercase text-slate-400 tracking-widest mb-1">Tanggal Transfer</p>
                            <p class="text-sm font-black text-slate-700"><?php echo e($paymentProof->transfer_date->format('d M Y')); ?></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <?php if($paymentProof->status === 'UPLOADED'): ?>
            <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 space-y-6">
                ...
                <form action="<?php echo e(route('finance.payment-proofs.review', $paymentProof)); ?>" method="POST" onsubmit="return confirm('Setujui pembayaran ini?')">
                    <?php echo csrf_field(); ?>
                ...
                <form action="<?php echo e(route('finance.payment-proofs.review', $paymentProof)); ?>" method="POST" class="space-y-4">
                    <?php echo csrf_field(); ?>
            ...
            <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8">
                ...
                <div class="p-6 rounded-[2rem] <?php echo e($paymentProof->status === 'APPROVED' ? 'bg-emerald-50 text-emerald-800' : 'bg-red-50 text-red-800'); ?>">
                    <p class="text-[10px] font-black uppercase tracking-widest mb-1 opacity-60">Status Dokument</p>
                    <p class="text-xl font-black"><?php echo e($paymentProof->status); ?></p>
                    <?php if($paymentProof->finance_notes): ?>
                        <div class="mt-4 pt-4 border-t border-current/10">
                            ...
                            <p class="text-sm font-bold italic">"<?php echo e($paymentProof->finance_notes); ?>"</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <div class="lg:col-span-8">
            <div class="bg-slate-900 rounded-[3rem] border-8 border-slate-800 shadow-2xl overflow-hidden relative group min-h-[600px] flex items-center justify-center">
                ...
                <?php if($paymentProof->file_path): ?>
                    <?php if($paymentProof->is_pdf): ?>
                        <div class="w-full h-full min-h-[850px] flex flex-col">
                            <iframe src="<?php echo e($paymentProof->file_url); ?>" 
                                    class="w-full h-[850px] rounded-2xl border-none shadow-2xl"
                                    allow="autoplay"></iframe>
                            <div class="p-4 flex justify-center bg-slate-800/50">
                                <a href="<?php echo e($paymentProof->file_url); ?>" target="_blank" class="text-xs font-black text-white/70 hover:text-white uppercase tracking-[0.2em] flex items-center gap-2 transition-colors">
                    ...
                    <?php else: ?>
                        <img src="<?php echo e($paymentProof->file_url); ?>" 
            ...
            <div class="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4">
                <a href="<?php echo e($paymentProof->file_url); ?>" target="_blank" 
                   class="bg-white rounded-3xl p-6 border border-slate-100 flex items-center gap-4 hover:border-[#8B1538] transition-all group">
                    ...
                    <div>
                        ...
                        <p class="text-xs font-black text-slate-700 break-all"><?php echo e(basename($paymentProof->file_path)); ?></p>
                    </div>
                </a>
                <div class="bg-white rounded-3xl p-6 border border-slate-100 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs font-black text-slate-700"><?php echo e($paymentProof->created_at->format('d M Y, H:i')); ?></p>
                    </div>
                </div>
            </div>
        </div>
...
<?php echo $__env->make('layouts.finance', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/naradata/STIH-SIAKAD/resources/views/finance/payment-proofs/show.blade.php ENDPATH**/ ?>