<?php $__env->startSection('page-title', 'Pengajuan Cicilan'); ?>
                                ...
                                <td class="px-8 py-6">
                                    <div class="flex items-center gap-4">
                                        <div class="size-12 rounded-2xl bg-slate-100 border border-slate-200 overflow-hidden shrink-0 group-hover:scale-110 transition-transform">
                                            <img src="https://ui-avatars.com/api/?name=<?php echo e(urlencode($request->student->user->name ?? 'User')); ?>&background=8B1538&color=fff&bold=true" alt="Avatar">
                                        </div>
                                        <div>
                                            <p class="text-sm font-black text-slate-700 leading-tight"><?php echo e($request->student->user->name ?? 'N/A'); ?></p>
                                            <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-1"><?php echo e($request->student->nim ?? 'N/A'); ?></p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-8 py-6">
                                    <div class="flex flex-col">
                                        <span class="text-sm font-black text-[#8B1538] tracking-tight">Rp <?php echo e(number_format($request->invoice->total_tagihan, 0, ',', '.')); ?></span>
                                        <span class="text-[10px] font-black uppercase text-slate-400 tracking-widest mt-1">
                                            Semester <?php echo e($request->invoice->semester); ?> (<?php echo e($request->invoice->tahun_ajaran); ?>)
                                        </span>
                                    </div>
                                </td>
                                <td class="px-8 py-6 text-center">
                                    <div class="inline-flex flex-col items-center px-4 py-2 bg-[#8B1538]/5 rounded-2xl border border-[#8B1538]/10">
                                        <span class="text-xl font-black text-[#8B1538] leading-none"><?php echo e($request->requested_terms); ?></span>
                                        <span class="text-[8px] font-black uppercase tracking-tighter text-[#8B1538] opacity-60">Kali Cicilan</span>
                                    </div>
                                </td>
                                <td class="px-8 py-6 text-right">
                                    <a href="<?php echo e(route('finance.installment-requests.show', $request)); ?>" 
...
<?php echo $__env->make('layouts.finance', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /Users/naradata/STIH-SIAKAD/resources/views/finance/installment-requests/index.blade.php ENDPATH**/ ?>