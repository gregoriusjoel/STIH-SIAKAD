    ...
    @section('navbar_breadcrumb')
        <nav class="flex items-center gap-3 text-sm text-white/60 font-bold tracking-tight">
            ...
            <a href="{{ route('dosen.kelas.detail', $class_info['id']) }}"
                class="hover:text-white transition-all duration-300">{{ $class_info['name'] }}</a>
            <span class="material-symbols-outlined text-[10px] text-white/20 font-normal">play_arrow</span>
            <span class="text-white font-black text-[13px] uppercase tracking-wider">Input Nilai Akhir</span>
        </nav>
    @endsection

    <div class="pt-4 px-4 md:pt-6 md:px-8 pb-8 w-full flex flex-col gap-6" x-data="nilaiApp()">

        <!-- Header -->
        <div class="flex flex-col gap-4">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 class="text-2xl md:text-3xl font-black text-[#111218] dark:text-white tracking-tight">Nilai Akhir
                        Mahasiswa</h2>
                    <p class="text-sm md:text-base text-[#616889] dark:text-slate-400 mt-1">
                        <span x-text="classInfo.name"></span> (<span x-text="classInfo.code"></span>) - <span
                            x-text="classInfo.class_name"></span>
                    </p>
                </div>
                <a href="{{ route('dosen.kelas.detail', $class_info['id']) }}"
                ...
                @foreach(['uts' => 'UTS', 'uas' => 'UAS'] as $key => $label)
                    ...
                    @if($ps)
                        <div
                            ...
                            <div class="flex-1">
                                <p
                                    class="text-sm font-bold {{ $ps['status'] === 'active' ? 'text-green-900 dark:text-green-200' : ($ps['status'] === 'upcoming' ? 'text-blue-900 dark:text-blue-200' : 'text-amber-900 dark:text-amber-200') }}">
                                    Periode {{ $label }}: {{ $ps['label'] }}
                                </p>
                                <p
                                    class="text-xs {{ $ps['status'] === 'active' ? 'text-green-700 dark:text-green-300' : ($ps['status'] === 'upcoming' ? 'text-blue-700 dark:text-blue-300' : 'text-amber-700 dark:text-amber-300') }}">
                                    {{ $ps['message'] }}
                                </p>
                            </div>
                            <span
        ...
        <div class="bg-white dark:bg-[#1a1d2e] rounded-2xl border border-gray-200 dark:border-slate-800 shadow-sm p-4 md:p-6"
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                ...
                <span
                    class="self-start md:self-auto text-xs font-semibold text-amber-600 bg-amber-50 dark:bg-amber-900/20 px-3 py-1 rounded-full">
                    Belum Dikunci
                </span>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4 mb-6">
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2">
                        Partisipatif (%)</label>
                    <input type="number" x-model.number="bobot.bobot_partisipatif" @input="updateTotal" min="0" max="100"
                        step="0.01"
                ...
                <div>
                    ...
                    <input type="number" x-model.number="bobot.bobot_proyek" @input="updateTotal" min="0" max="100"
                        step="0.01"
                        class="block w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent dark:bg-slate-800 dark:text-white">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2">Quiz (%)</label>
                    <input type="number" x-model.number="bobot.bobot_quiz" @input="updateTotal" min="0" max="100"
                        step="0.01"
                        class="block w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent dark:bg-slate-800 dark:text-white">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2">Tugas (%)</label>
                    <input type="number" x-model.number="bobot.bobot_tugas" @input="updateTotal" min="0" max="100"
                        step="0.01"
                        class="block w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent dark:bg-slate-800 dark:text-white">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2">UTS (%)</label>
                    <input type="number" x-model.number="bobot.bobot_uts" @input="updateTotal" min="0" max="100" step="0.01"
                        class="block w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent dark:bg-slate-800 dark:text-white">
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2">UAS (%)</label>
                    <input type="number" x-model.number="bobot.bobot_uas" @input="updateTotal" min="0" max="100" step="0.01"
                        class="block w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent dark:bg-slate-800 dark:text-white">
                </div>
            </div>

            <div
                class="flex flex-col md:flex-row md:items-center justify-between gap-4 pt-4 border-t border-gray-200 dark:border-slate-700">
                <div>
                    <p class="text-sm font-medium text-gray-600 dark:text-slate-400">Total Bobot:</p>
                    <p class="text-2xl font-bold" :class="totalBobot === 100 ? 'text-green-600' : 'text-red-600'"
                        x-text="totalBobot.toFixed(2) + '%'"></p>
                </div>
                <button @click="saveBobot" :disabled="totalBobot !== 100 || isSaving"
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <div class="w-12 h-12 rounded-xl bg-green-50 dark:bg-green-900/20 flex items-center justify-center">
                        <span
                            class="material-symbols-outlined text-2xl text-green-600 dark:text-green-400">check_circle</span>
                    </div>
                    <div>
                ...
                <div class="flex gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-amber-800 dark:text-amber-300">
                            Mengubah bobot akan <strong>otomatis menghitung ulang semua nilai mahasiswa</strong> yang sudah
                            diinput.
                        </p>
                    </div>
                </div>
        ...
        <div class="bg-white dark:bg-[#1a1d2e] rounded-2xl border border-gray-200 dark:border-slate-800 shadow-sm overflow-hidden"
            ...
            <div
                class="px-4 py-4 md:px-6 nav-header border-b border-gray-200 dark:border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div class="flex items-center gap-2">
                ...
                <div class="flex flex-col md:flex-row gap-3 w-full md:w-auto">
                    <button @click="tarikSemuaNilai" :disabled="isSaving || !hasPublishedGrades" x-show="hasPublishedGrades"
                        x-transition
                    ...
                    <div x-show="isAutoSaving" x-transition
                    ...
                    <button @click="saveAllNilai" :disabled="isSaving"
            ...
            <div class="overflow-x-auto">
                <table class="w-full divide-y divide-gray-200 dark:divide-slate-700">
                    <thead class="bg-gray-50 dark:bg-slate-800">
                        <tr>
                            <th
                                class="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-slate-300 uppercase tracking-wider whitespace-nowrap">
                                No</th>
                            <th
                                class="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-slate-300 uppercase tracking-wider whitespace-nowrap">
                                NIM</th>
                            <th
                                class="px-4 py-3 text-left text-xs font-bold text-gray-700 dark:text-slate-300 uppercase tracking-wider whitespace-nowrap w-full min-w-[200px]">
                                Nama Mahasiswa</th>
                            <th
                                class="px-4 py-3 text-center text-xs font-bold text-gray-700 dark:text-slate-300 uppercase tracking-wider whitespace-nowrap">
                                Partisipatif</th>
                            <th
                                class="px-4 py-3 text-center text-xs font-bold text-gray-700 dark:text-slate-300 uppercase tracking-wider whitespace-nowrap">
                                Proyek</th>
                            ...
                            <th class="px-4 py-3 text-center text-xs font-bold uppercase tracking-wider whitespace-nowrap"
                                ...
                                <div class="flex items-center justify-center gap-1">
                                    UTS
                                    <span x-show="periodStatuses.uts.status === 'active'"
                                        class="material-symbols-outlined text-[14px] text-green-500">lock_open</span>
                                    <span x-show="periodStatuses.uts.status === 'closed'"
                                        class="material-symbols-outlined text-[14px] text-amber-500">lock</span>
                                </div>
                            </th>
                            <th class="px-4 py-3 text-center text-xs font-bold uppercase tracking-wider whitespace-nowrap"
                                ...
                                <div class="flex items-center justify-center gap-1">
                                    UAS
                                    <span x-show="periodStatuses.uas.status === 'active'"
                                        class="material-symbols-outlined text-[14px] text-green-500">lock_open</span>
                                    <span x-show="periodStatuses.uas.status === 'closed'"
                                        class="material-symbols-outlined text-[14px] text-amber-500">lock</span>
                                </div>
                            </th>
                            <th
                                class="px-4 py-3 text-center text-xs font-bold text-primary uppercase tracking-wider whitespace-nowrap">
                                Nilai Akhir</th>
                            <th
                                class="px-4 py-3 text-center text-xs font-bold text-primary uppercase tracking-wider whitespace-nowrap">
                                Grade</th>
                        ...
                        <template x-for="(student, index) in students" :key="student.krs_id">
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors"
                                ...
                                <td class="px-4 py-3 text-sm font-medium text-gray-900 dark:text-white"
                                    x-text="student.nim"></td>
                                <td class="px-4 py-3 text-sm text-gray-900 dark:text-white">
                                    <div class="flex items-center gap-2">
                                        <span x-text="student.name"></span>
                                        <template x-if="student.is_internship">
                                            <span
                                                class="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300">Magang</span>
                                        </template>
                                    </div>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <template x-if="!student.is_internship">
                                        <input type="number" x-model.number="student.nilai_partisipatif"
                                            @input="validateGrade(student, 'nilai_partisipatif')" min="0" max="100"
                                            step="0.01"
                                ...
                                <td class="px-4 py-3 text-center">
                                    <template x-if="!student.is_internship">
                                        <input type="number" x-model.number="student.nilai_proyek"
                                            @input="validateGrade(student, 'nilai_proyek')" min="0" max="100" step="0.01"
                                            class="w-20 px-2 py-1 text-center border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary dark:bg-slate-700 dark:text-white">
                                    </template>
                                    <template x-if="student.is_internship">
                                        <span class="text-xs text-gray-400 dark:text-slate-500 italic">—</span>
                                ...
                                <td class="px-4 py-3 text-center">
                                    <template x-if="!student.is_internship">
                                        <input type="number" x-model.number="student.nilai_quiz"
                                            @input="validateGrade(student, 'nilai_quiz')" min="0" max="100" step="0.01"
                                            class="w-20 px-2 py-1 text-center border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary dark:bg-slate-700 dark:text-white">
                                    </template>
                                    <template x-if="student.is_internship">
                                        <span class="text-xs text-gray-400 dark:text-slate-500 italic">—</span>
                                ...
                                <td class="px-4 py-3 text-center">
                                    <template x-if="!student.is_internship">
                                        <input type="number" x-model.number="student.nilai_tugas"
                                            @input="validateGrade(student, 'nilai_tugas')" min="0" max="100" step="0.01"
                                            class="w-20 px-2 py-1 text-center border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary dark:bg-slate-700 dark:text-white">
                                    </template>
                                    <template x-if="student.is_internship">
                                        <span class="text-xs text-gray-400 dark:text-slate-500 italic">—</span>
                                ...
                                <td class="px-4 py-3 text-center">
                                    <template x-if="!student.is_internship">
                                        <input type="number" x-model.number="student.nilai_uts"
                                            @input="validateGrade(student, 'nilai_uts')" min="0" max="100" step="0.01"
                                ...
                                <td class="px-4 py-3 text-center">
                                    <template x-if="!student.is_internship">
                                        <input type="number" x-model.number="student.nilai_uas"
                                            @input="validateGrade(student, 'nilai_uas')" min="0" max="100" step="0.01"
                                            class="w-20 px-2 py-1 text-center rounded-lg focus:ring-2 focus:ring-primary dark:bg-slate-700 dark:text-white"
                                ...
                                <td class="px-4 py-3 text-center">
                                    <span class="text-sm font-bold text-primary"
                                        x-text="student.nilai_akhir.toFixed(2)"></span>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold"
                                        :class="getGradeColor(student.grade)" x-text="student.grade"></span>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <span class="text-sm font-bold text-gray-900 dark:text-white"
                                        x-text="student.bobot.toFixed(2)"></span>
                                </td>
                            </tr>
                        </template>
                    </tbody>
        ...
        <div x-show="showImportModal" x-transition.opacity
            ...
            @click.self="showImportModal = false" x-cloak>
            <div class="bg-white dark:bg-[#1a1d2e] rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden" @click.stop>
                ...
                <div class="px-6 py-5 space-y-4">
                    ...
                    <div class="border-2 border-dashed border-gray-300 dark:border-slate-600 rounded-xl p-6 text-center"
                        ...
                        <input type="file" accept=".csv,.txt" @change="importFile = $event.target.files[0]"
                            x-ref="importFileInput" class="hidden">
                ...
                <div class="px-6 py-4 border-t border-gray-200 dark:border-slate-700 flex justify-end gap-3">
                    ...
                    <button @click="executeImport()" :disabled="!importFile || isImporting"
                        ...
                        <svg x-show="isImporting" class="animate-spin h-4 w-4" xmlns="http://www.w3.org/2000/svg"
