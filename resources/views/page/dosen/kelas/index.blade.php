            ...
            <div class="bg-white rounded-xl border border-gray-200 p-6 relative hover:shadow-sm transition-shadow duration-300">
                ...
                <div class="mb-6">
                    <div class="flex justify-between text-xs mb-2">
                        <span class="text-[#616889] font-medium">Progress Materi</span>
                        <span class="text-[#8B1538] font-bold">{{ ($class['progress'] ?? 0) }}%</span>
                    </div>
                    <div class="w-full bg-gray-100 rounded-full h-1.5">
                        <div class="bg-[#8B1538] h-1.5 rounded-full" style="width: {{ ($class['progress'] ?? 0) }}%"></div>
                    </div>
                </div>

                <div class="flex gap-3">
                        <a href="{{ route('dosen.kelas.absensi', $class['id']) }}" onclick="event.preventDefault(); if(!openModal(this.href)) window.location.href = this.href;" class="flex-1 text-center py-2 rounded-lg border border-gray-200 text-gray-600 text-sm font-medium hover:bg-gray-50 transition-colors">
                        Absensi
                    </a>
                    <a href="{{ route('dosen.kelas.detail', $class['id']) }}" class="flex-1 text-center py-2 rounded-lg border border-[#8B1538] text-[#8B1538] text-sm font-medium hover:bg-[#FEF2F2] transition-colors">
                        Detail
                    </a>
                </div>
            </div>
