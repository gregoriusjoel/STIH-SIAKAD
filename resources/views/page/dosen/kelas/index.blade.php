...
@section('content')
    <div class="p-6 md:p-8 max-w-[1400px] mx-auto w-full flex flex-col gap-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-[#616889] dark:text-slate-400 mt-1 text-sm">Kelola materi dan aktivitas perkuliahan untuk semester ini.</p>
            </div>
        ...
        <div id="classGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($classes as $class)
                ...
                <div class="kelas-card bg-white dark:bg-[#1a1d2e] rounded-2xl border border-gray-200 dark:border-slate-800 p-6 flex flex-col gap-4 shadow-sm hover:shadow-md transition-all duration-300 group" data-search="{{ $searchableText }}">
                    ...
                    <div>
                        <h3 class="text-xl font-bold text-[#111218] dark:text-white mb-1 tracking-tight group-hover:text-primary transition-colors">{{ $class['name'] }}</h3>
                        <p class="text-sm text-[#616889] dark:text-slate-400 font-medium">{{ $class['code'] }} • {{ $class['sks'] }} SKS</p>
                    </div>
                    ...
                    <div class="mt-2 pt-4 border-t border-gray-100 dark:border-slate-800">
                        ...
                        <div class="w-full bg-gray-100 dark:bg-slate-800 rounded-full h-2 overflow-hidden">
                            ...
                            @endphp
                            <div class="h-full rounded-full bg-gradient-to-r from-primary to-pink-400" style="width: {{ $percent }}%"></div>
                        </div>
                    </div>

                    <div class="grid grid-cols-2 gap-3 mt-2">
                        <a href="{{ route('dosen.kelas.absensi', $class['id']) }}" class="flex items-center justify-center py-2.5 rounded-xl border border-gray-200 dark:border-slate-700 text-gray-600 dark:text-gray-300 text-sm font-bold hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors">
                            Absensi
                        </a>
                        <a href="{{ route('dosen.kelas.detail', $class['id']) }}" class="flex items-center justify-center py-2.5 rounded-xl bg-primary text-white text-sm font-bold hover:bg-primary-hover shadow-lg shadow-primary/20 transition-all">
                            Detail
                        </a>
                    </div>
                </div>
    ...
    <div id="absensiModal" class="fixed inset-0 z-[60] hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm transition-opacity" aria-hidden="true" onclick="closeAbsensiModal()"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
            <div class="relative z-10 inline-block align-bottom bg-white dark:bg-[#1a1d2e] rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full border border-gray-100 dark:border-slate-700">
