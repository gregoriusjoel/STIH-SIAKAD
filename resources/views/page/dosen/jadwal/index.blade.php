                    ...
                    <div
                        ...
                        @if(isset($schedulesByDay[$day]) && $schedulesByDay[$day]->count() > 0)
                            @foreach($schedulesByDay[$day] as $jadwal)
                                <!-- Class Item -->
                                <div
                                    class="flex gap-3 relative pl-3 hover:bg-gray-50/50 dark:hover:bg-slate-800/30 rounded-r-lg transition-colors p-2 -mx-2">
                                    ...
                                    <div class="flex-1 flex flex-col gap-1">
                                        <div class="flex justify-between items-start">
                                            <h4 class="font-bold text-[#111218] dark:text-white text-sm">
                                                {{ $jadwal->kelas->mataKuliah->nama_mk }}</h4>
                                            <span
                                                class="bg-{{ ($jadwal->kelas->mataKuliah->jenis ?? 'Teori') === 'Praktikum' ? 'blue' : 'orange' }}-50 text-{{ ($jadwal->kelas->mataKuliah->jenis ?? 'Teori') === 'Praktikum' ? 'blue' : 'orange' }}-600 px-2 py-0.5 rounded text-[10px] font-bold">{{ $jadwal->kelas->mataKuliah->jenis ?? 'Teori' }}</span>
                                        </div>
                                        <div class="flex items-center gap-2 text-xs text-[#616889]">
                                            <span
                                                class="bg-red-50 text-red-700 px-1.5 rounded font-bold">{{ $jadwal->kelas->section }}</span>
                                            <span>•</span>
