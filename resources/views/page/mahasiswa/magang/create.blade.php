...
@section('content')
@php
    $selectedTypeId = request('internship_type_id') ?? old('internship_type_id') ?? ($internshipTypes->first()?->id ?? 1);
    $selectedType = $internshipTypes->firstWhere('id', $selectedTypeId) ?? $internshipTypes->first();
@endphp
<div class="max-w-[1600px] mx-auto px-4 py-8">

    {{-- Back action --}}
    <div class="mb-6">
        <a href="{{ route('mahasiswa.magang.index') }}" class="inline-flex items-center gap-2 text-sm font-semibold text-gray-500 hover:text-[#8B1538] transition-colors group">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-[2rem] shadow-sm border border-gray-100 dark:border-gray-700 p-8 lg:p-12 relative overflow-hidden">
        ...
        <form method="POST" action="{{ route('mahasiswa.magang.store') }}" enctype="multipart/form-data" class="space-y-8">
            @csrf
        ...
        <div class="p-6 rounded-2xl bg-gradient-to-r {{ $selectedType->is_conversion ? 'from-purple-500/5 to-indigo-500/5 border-purple-100' : 'from-teal-500/5 to-emerald-500/5 border-teal-100' }} border-2 flex items-start gap-4">
            ...
            <div class="flex-1 min-w-0">
                <p class="text-[10px] font-bold {{ $selectedType->is_conversion ? 'text-purple-500' : 'text-teal-600' }} uppercase tracking-widest leading-none">Tipe Program Terpilih</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="instansi" value="{{ old('instansi') }}" required placeholder="Contoh: PT. Adhyaksa Corp"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="alamat_instansi" rows="2" required placeholder="Alamat lengkap instansi tempat magang"
                              class="w-full rounded-2xl border-gray-200 bg-gray-50 dark:border-gray-600 dark:bg-gray-700 dark:text-white px-5 py-3.5 text-sm focus:bg-white transition-colors focus:ring-4 focus:ring-red-900/10 focus:border-[#8B1538]">{{ old('alamat_instansi') }}</textarea>
                </div>

                <div class="md:col-span-2">
                    <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Posisi / Bagian</label>
                    <input type="text" name="posisi" value="{{ old('posisi') }}" placeholder="Contoh: Legal Officer Intern"
                           class="w-full rounded-2xl border-gray-200 bg-gray-50 dark:border-gray-600 dark:bg-gray-700 dark:text-white px-5 py-3.5 text-sm focus:bg-white transition-colors focus:ring-4 focus:ring-red-900/10 focus:border-[#8B1538]">
                </div>
            </div>
            ...
            <div class="p-6 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700"
                 x-data="{
                    startDate: '{{ old('periode_mulai') }}',
                    endDate: '{{ old('periode_selesai') }}',
                    durasi: {{ old('periode_selesai') ? 0 : 0 }},
                    custom: false,
                    customMonth: '',
                    setDurasi(n) {
                        this.custom = (n === 'other');
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="3" placeholder="Jelaskan secara singkat rencana kegiatan / tugas magang Anda"
                          class="w-full rounded-2xl border-gray-200 bg-gray-50 dark:border-gray-600 dark:bg-gray-700 dark:text-white px-5 py-3.5 text-sm focus:bg-white transition-colors focus:ring-4 focus:ring-red-900/10 focus:border-[#8B1538]">{{ old('deskripsi') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-2 border-t border-gray-100 dark:border-gray-700">
                <div>
                    ...
                    <input type="text" name="pembimbing_lapangan_nama" value="{{ old('pembimbing_lapangan_nama') }}" placeholder="Nama Mentor / Supervisor Instansi"
                           class="w-full rounded-2xl border-gray-200 bg-gray-50 dark:border-gray-600 dark:bg-gray-700 dark:text-white px-5 py-3.5 text-sm focus:bg-white transition-colors focus:ring-4 focus:ring-red-900/10 focus:border-[#8B1538]">
                </div>
                <div>
