...
<div class="px-4 py-6 max-w-4xl mx-auto space-y-6 font-inter" x-data="{ 
    ...
    <div class="flex items-center gap-4 mb-6">
        <a href="{{ route('mahasiswa.prestasi.show', $prestasi) }}" class="w-10 h-10 rounded-full bg-white shadow-sm flex items-center justify-center border border-gray-100 text-gray-500 hover:text-[#7a1621] hover:border-red-100 hover:bg-red-50 transition-colors">
    ...
    <form method="POST" action="{{ route('mahasiswa.prestasi.update', $prestasi) }}" class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        @csrf
        @method('PUT')
        ...
        <div class="p-6 sm:p-8 space-y-8">
            ...
            <div>
                ...
                <div>
                    ...
                    <select name="dosen_pendamping_id" class="w-full rounded-xl border-gray-300 text-sm px-4 py-2.5 focus:ring-[#7a1621] focus:border-[#7a1621]">
                        <option value="">-- Tidak ada dosen pendamping --</option>
                        @foreach($dosens as $dosen)
                            <option value="{{ $dosen->id }}" {{ old('dosen_pendamping_id', $prestasi->dosen_pendamping_id) == $dosen->id ? 'selected' : '' }}>
        ...
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-end gap-3">
            <a href="{{ route('mahasiswa.prestasi.show', $prestasi) }}" class="px-5 py-2.5 text-sm font-bold text-gray-600 hover:bg-gray-200 rounded-xl transition-colors">Batal</a>
            <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-[#7a1621] to-[#6D1029] hover:from-[#6D1029] hover:to-[#4e091b] text-white text-sm font-bold rounded-xl shadow-sm transition-all flex items-center gap-2">
