...
<div class="px-4 py-6 max-w-[1600px] mx-auto font-inter" x-data="{ showUploadModal: false }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
        <a href="{{ route('mahasiswa.prestasi.index') }}" class="inline-flex items-center gap-2 text-sm font-semibold text-gray-500 hover:text-[#7a1621] transition-colors group w-fit">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
            ...
            {{-- Aksi Mahasiswa --}}
            @if($prestasi->isEditable() || in_array($prestasi->status, [\App\Models\Prestasi::STATUS_DRAFT, \App\Models\Prestasi::STATUS_DITOLAK]))
            <div class="bg-indigo-50/80 border border-indigo-200/60 rounded-2xl p-6 shadow-sm">
                ...
                <div class="flex flex-wrap gap-3">
                    @if($prestasi->isEditable())
                        <a href="{{ route('mahasiswa.prestasi.edit', $prestasi) }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 text-sm font-bold rounded-xl transition shadow-sm">
                    ...
                    @if(in_array($prestasi->status, [\App\Models\Prestasi::STATUS_DRAFT, \App\Models\Prestasi::STATUS_DITOLAK]))
                        <form method="POST" action="{{ route('mahasiswa.prestasi.submit', $prestasi) }}" class="inline">
                            @csrf
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    @if(in_array($prestasi->status, [\App\Models\Prestasi::STATUS_DRAFT, \App\Models\Prestasi::STATUS_DITOLAK, \App\Models\Prestasi::STATUS_DIPROSES_ADMIN]))
                ...
                <div class="space-y-3">
                    @foreach($prestasi->surats as $surat)
                        <div class="p-4 bg-gray-50 border border-gray-200 rounded-xl flex items-center justify-between gap-4">
                            ...
                            <a href="{{ route('mahasiswa.prestasi.surat.download', [$prestasi, $surat]) }}" class="shrink-0 px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-bold text-gray-700 hover:text-[#7a1621] hover:border-[#7a1621] transition-colors shadow-sm flex items-center gap-2">
    ...
    <div x-show="showUploadModal" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto overflow-x-hidden bg-gray-900/50 backdrop-blur-sm p-4">
        <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl" @click.away="showUploadModal = false">
            ...
            <form method="POST" action="{{ route('mahasiswa.prestasi.upload-dokumen', $prestasi) }}" enctype="multipart/form-data">
                @csrf
