...
<div class="container">
    ...
    <div class="muted" style="text-align:center; margin-bottom:4px;">
        {{ $semesterName }} - {{ $semesterAktif->tahun_ajaran ?? '-' }}
    </div>

    <table class="meta-table">
        <tr>
            <td style="width: 14%;">NIM</td>
            <td style="width: 36%;">{{ $mahasiswa->nim ?? '-' }}</td>
            <td style="width: 14%;">Tahun Ajaran</td>
            <td style="width: 36%;">{{ $semesterAktif->tahun_ajaran ?? '-' }}</td>
        </tr>
        <tr>
            <td>Nama</td>
            <td>{{ $mahasiswa->user->name ?? '-' }}</td>
            <td>Semester</td>
            <td>Semester {{ $semesterNumber }} ({{ $semesterName }})</td>
        </tr>
        <tr>
            <td>Dosen PA</td>
        ...
        <tbody>
            @forelse($existingKrs as $i => $k)
                ...
                <tr>
                    ...
                    <td class="text-center">{{ $jamLabel }}</td>
                    <td>{{ $ruanganLabel }}</td>
                    <td>{{ $dosenName }}</td>
                </tr>
            @empty
        ...
        <table class="signature-table">
            <tr>
                <td>
                    ...
                    <div class="muted">({{ optional($mahasiswa->dosen_pa)->user->name ?? '................' }})</div>
                </td>
                <td>
                    <div class="muted">Jakarta, {{ \Carbon\Carbon::now()->format('d F Y') }}</div>
                    <div class="muted">Mahasiswa</div>
                    <div class="sign-line"></div>
                    <div class="muted">({{ $mahasiswa->user->name ?? '-' }})</div>
                </td>
            </tr>
        </table>
    </div>
    ...
    <div class="footer-note">
        *KRS ini dicetak otomatis oleh sistem SIAKAD pada {{ \Carbon\Carbon::now()->format('d-m-Y H:i:s') }}.
    </div>
</div>
</body>
</html>
