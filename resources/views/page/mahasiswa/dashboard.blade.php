    ...
    <div class="space-y-6">
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-red-800 mb-3">
                            Anda harus melengkapi data profil terlebih dahulu untuk dapat mengakses fitur KRS, Nilai, Jadwal,
                            dan fitur lainnya.
                        </p>
        ...
        <div class="bg-gradient-to-r from-maroon to-maroon-hover rounded-xl shadow-lg text-white p-8">
            <div class="flex items-center gap-6">
                <div
                    class="w-20 h-20 rounded-full overflow-hidden bg-white/20 flex items-center justify-center text-3xl font-bold">
                    @if(!empty($mahasiswa->foto))
                        <img src="{{ asset('storage/' . $mahasiswa->foto) }}" alt="Foto {{ $mahasiswa->user->name }}"
                ...
                <div class="flex-1">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        <div><span class="opacity-80">NIM:</span> <span class="font-semibold">{{ $mahasiswa->nim }}</span>
                        </div>
                        <div><span class="opacity-80">Prodi:</span> <span
                                class="font-semibold">{{ $mahasiswa->prodi ?? 'Hukum' }}</span></div>
                        <div><span class="opacity-80">Angkatan:</span> <span
                                class="font-semibold">{{ $mahasiswa->angkatan ?? '2023' }}</span></div>
                        <div class="mt-1"><span class="opacity-80">Semester:</span> <span
                                class="font-semibold">{{ $studentSemester }}</span></div>
                        <div>
                            <span class="opacity-80">Status:</span>
                            <span
                                class="px-2 py-1 rounded-full text-xs font-bold ml-1 {{ $mahasiswa->status === 'aktif' ? 'bg-green-400 text-green-900' : 'bg-yellow-400 text-yellow-900' }}">
                ...
                <div class="p-4 text-sm text-gray-700">
                    ...
                    <div class="mt-4">
                        @if($paList->isNotEmpty())
                            <a href="#" class="text-sm text-red-700 font-semibold">Klik disini untuk melihat kontak PA (WA
                                Only!).</a>
                        @else
                            <a href="{{ Route::has('mahasiswa.kontak.pa') ? route('mahasiswa.kontak.pa') : '#' }}"
                                class="text-sm text-red-700 font-semibold">Klik disini untuk melihat kontak PA (WA Only!).</a>
                        @endif
                    </div>
                </div>
            </div>
