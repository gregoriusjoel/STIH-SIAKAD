        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <div class="p-6 border-b border-gray-200">
                ...
                <p class="text-sm text-gray-600 mt-1">
                    Nama kelas akan di-generate otomatis dari kombinasi Angkatan, Kode Prodi, dan Kode Kelas.
                </p>
            </div>

            <form method="POST" action="{{ route('admin.kelas-perkuliahan.store') }}" class="p-6" x-data="{
                angkatan: '{{ old('angkatan', date('Y')) }}',
                ...
                @csrf
                <input type="hidden" name="tingkat" value="{{ old('tingkat', 0) }}">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-6">
                    <div class="lg:col-span-8 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="angkatan" id="angkatan" x-model="angkatan" required
                                    ...
                                    @foreach($angkatanOptions as $year)
                                        <option value="{{ $year }}" {{ old('angkatan', date('Y')) == $year ? 'selected' : '' }}>
                                            {{ $year }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                <label for="tahun_akademik_id" class="block text-sm font-semibold text-gray-700 mb-2">
                                    Tahun Akademik
                                </label>
                                <select name="tahun_akademik_id" id="tahun_akademik_id"
                                    ...
                                    @foreach($semesters as $sem)
                                        <option value="{{ $sem->id }}" {{ old('tahun_akademik_id') == $sem->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <input type="text" name="kode_kelas" id="kode_kelas" x-model="kodeKelas"
                                    value="{{ old('kode_kelas', '01') }}" placeholder="01" required maxlength="2"
                        ...
                        <div class="sticky top-6">
                            ...
                            <div
                                ...
                                <div class="flex items-center justify-center gap-2 text-xs font-medium text-blue-600/70 tracking-wider">
                                    <span>[2 DIGIT ANGKATAN]</span><i class="fas fa-plus text-[10px]"></i>
                            ...
                            <div class="bg-blue-50/80 border border-blue-100 rounded-lg p-4 text-sm text-blue-800 mt-6 leading-relaxed">
                                ...
                                Jika kombinasi <strong>Angkatan + Prodi + Kode Kelas + Tahun Akademik</strong> sudah ada,
                                sistem akan menolak data duplikat.
                            </div>
                        </div>
                    </div>
                </div>
