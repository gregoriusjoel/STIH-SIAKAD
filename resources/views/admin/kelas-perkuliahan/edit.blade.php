        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <div class="p-6 border-b border-gray-200">
                ...
                <p class="text-sm text-gray-600 mt-1">
                    Mengedit data kelas <strong>{{ $kelasPerkuliahan->nama_kelas }}</strong>
                </p>
            </div>

            <form method="POST" action="{{ route('admin.kelas-perkuliahan.update', $kelasPerkuliahan) }}"
                class="p-6 space-y-5" x-data="{
                    angkatan: '{{ old('angkatan', $kelasPerkuliahan->angkatan) }}',
                    ...
                    get previewNamaKelas() {
                        if (!this.angkatan || !this.kodeProdi || !this.kodeKelas) return '...';
                        return String(this.angkatan).slice(-2) + this.kodeProdi.toUpperCase() + this.kodeKelas;
                    }
                }">
                @csrf
                @method('PUT')
                <input type="hidden" name="tingkat" value="{{ old('tingkat', $kelasPerkuliahan->tingkat ?? 0) }}">
                ...
                <div>
                    <label for="angkatan" class="block text-sm font-semibold text-gray-700 mb-1">
                        Angkatan / Tahun Masuk <span class="text-red-500">*</span>
                    </label>
                    <select name="angkatan" id="angkatan" x-model="angkatan" required
                        ...
                        @foreach($angkatanOptions as $year)
                            <option value="{{ $year }}" {{ old('angkatan', $kelasPerkuliahan->angkatan) == $year ? 'selected' : '' }}>
                                {{ $year }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label for="kode_prodi" class="block text-sm font-semibold text-gray-700 mb-1">
                ...
                <div>
                    <label for="kode_kelas" class="block text-sm font-semibold text-gray-700 mb-1">
                        Kode Kelas <span class="text-red-500">*</span>
                    </label>
                    <input type="text" name="kode_kelas" id="kode_kelas" x-model="kodeKelas" required maxlength="2"
                ...
                <div>
                    <label for="tahun_akademik_id" class="block text-sm font-semibold text-gray-700 mb-1">
                        Tahun Akademik
                    </label>
                    <select name="tahun_akademik_id" id="tahun_akademik_id"
                        ...
                        @foreach($semesters as $sem)
                            <option value="{{ $sem->id }}" {{ old('tahun_akademik_id', $kelasPerkuliahan->tahun_akademik_id) == $sem->id ? 'selected' : '' }}>
