        ...
        <div class="bg-white rounded-xl shadow-lg  overflow-hidden">
            <div class="p-6 border-b border-gray-200">
                ...
                <p class="text-sm text-gray-600 mt-1">Mengedit data kelas
                    <strong>{{ $kelasPerkuliahan->nama_kelas }}</strong></p>
            </div>
            ...
            <form method="POST" action="{{ route('admin.kelas-perkuliahan.update', $kelasPerkuliahan) }}"
                ...
                <div>
                    ...
                    <select name="tingkat" id="tingkat" x-model="tingkat" required
                        ...
                        @for($i = 1; $i <= 8; $i++)
                            <option value="{{ $i }}" {{ old('tingkat', $kelasPerkuliahan->tingkat) == $i ? 'selected' : '' }}>
                                Tingkat {{ $i }} (Semester {{ ($i * 2) - 1 }} & {{ $i * 2 }})
                            </option>
                        @endfor
                    </select>
                ...
                <div>
                    ...
                    <select name="tahun_akademik_id" id="tahun_akademik_id"
                        ...
                        @foreach($semesters as $sem)
                            <option value="{{ $sem->id }}" {{ old('tahun_akademik_id', $kelasPerkuliahan->tahun_akademik_id) == $sem->id ? 'selected' : '' }}>{{ $sem->nama_semester }}
                                {{ $sem->tahun_ajaran }}</option>
                        @endforeach
                    </select>
