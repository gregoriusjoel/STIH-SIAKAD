    ...
    <div class="px-4 py-6 md:px-8">
        ...
        <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="flex items-center space-x-3">
                <a href="{{ route('admin.jadwal_admin_approval.index') }}"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
                        ...
                        <span
                            class="px-3 py-1 bg-maroon/10 text-maroon text-xs font-bold rounded-full uppercase tracking-wider">
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <p class="text-sm text-gray-500">{{ $proposal->mataKuliah->kode_mk }} •
                                {{ $proposal->mataKuliah->sks }} SKS</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <div class="flex items-center mt-1">
                                <span
                                    class="px-3 py-1 bg-gray-100 text-gray-700 font-bold rounded-lg mr-2">{{ $proposal->kelas->resolved_kelas_name ?? '-' }}</span>
                                <span class="text-sm text-gray-500">Kapasitas: {{ $proposal->kelas->kapasitas ?? 0 }}
                                    Mahasiswa</span>
                            </div>
                        </div>
                        <div>
                            <p class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-1">Semester</p>
                            <p class="text-sm font-semibold text-gray-900">{{ $proposal->mataKuliah->semester }}
                                ({{ $proposal->mataKuliah->semester % 2 == 0 ? 'Genap' : 'Ganjil' }})</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="flex items-center p-4 bg-blue-50 rounded-xl">
                            ...
                            <div>
                                ...
                                <p class="text-lg font-bold text-gray-900">{{ substr($proposal->jam_mulai, 0, 5) }} -
                                    {{ substr($proposal->jam_selesai, 0, 5) }}</p>
                            </div>
                        </div>
                                        ...
                                        @foreach($proposal->approvals->sortByDesc('created_at') as $approval)
                                                        <tr class="text-sm">
                                                            ...
                                                            <td class="px-6 py-4">
                                                                ...
                                                                @endphp
                                            <span
                                                                    class="inline-flex items-center px-2 py-0.5 rounded-full font-medium {{ $statusColor[$approval->action] ?? 'bg-gray-100' }}">
                                                                    {{ $statusLabel[$approval->action] ?? $approval->action }}
                                                                </span>
                                                            </td>
                                                            <td class="px-6 py-4 text-gray-500">{{ $approval->created_at->format('d/m/Y H:i') }}
                                                            </td>
            ...
            <div class="space-y-6">
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <form action="{{ route('admin.jadwal_admin_approval.process', $proposal->id) }}" method="POST"
                        class="space-y-4">
                        @csrf
                        ...
                        <div>
                            ...
                            <select name="status" id="status_select"
                                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-maroon focus:border-transparent transition text-sm font-semibold"
                                required onchange="toggleChangesFields()">
                                @php $defaultStatus = $proposal->status === 'rejected_dosen' ? 'approved_with_changes' : 'approved'; @endphp
                                <option value="approved" {{ $defaultStatus === 'approved' ? 'selected' : '' }}>Setujui
                                    Pengajuan</option>
                                <option value="approved_with_changes" {{ $defaultStatus === 'approved_with_changes' ? 'selected' : '' }}>Setujui dengan Perubahan</option>
                                <option value="rejected" {{ $defaultStatus === 'rejected' ? 'selected' : '' }}>Tolak Pengajuan
                                </option>
                            </select>
                        </div>

                        <div id="changes_fields" class="space-y-4 pt-4 border-t border-gray-100 mt-4"
                            style="display: none;">
                            ...
                            @if(isset($dosenApproval) && ($dosenApproval->hari_pengganti || $dosenApproval->jam_mulai_pengganti))
                                <div class="mb-3">
                                    <button type="button" id="use_dosen_suggestion"
                                        class="px-3 py-2 bg-green-50 text-green-700 border border-green-200 rounded-lg text-sm font-semibold">Gunakan
                                        Usulan Dosen</button>
                            ...
                            <div class="grid grid-cols-1 gap-3">
                                <div class="relative">
                                    ...
                                    <div x-data="{
                                        ...
                                        <label
                                            class="flex items-center gap-2 text-[11px] font-bold text-gray-700 uppercase tracking-wider mb-2">
                                            <span class="material-symbols-outlined text-gray-400 text-sm">schedule</span>
                                            Jam Perkuliahan <span class="text-maroon">*</span>
                                        </label>
                                        <select name="jam_mulai" id="admin_jam_mulai" x-model="jamMulai" required
                                            class="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-maroon focus:ring-1 focus:ring-maroon transition-all font-medium">
                                            <option value="">Pilih Jam Perkuliahan</option>
                                            <template x-for="slot in validMulaiSlots" :key="slot.jam_mulai">
                                                <option :value="slot.jam_mulai.substring(0, 5)" x-text="slot.label">
                                                </option>
                                            </template>
                                        </select>
                                        <input type="hidden" name="jam_selesai" id="admin_jam_selesai" x-model="jamSelesai"
                                            required>
                                    </div>
                                </div>
                            </div>
                            <div>
                                ...
                                <select name="ruangan" id="admin_ruangan"
                                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-maroon focus:ring-1 focus:ring-maroon transition-all">
                        ...
                        <div class="pt-2">
                            ...
                            <textarea name="comments" rows="3"
                                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-maroon focus:border-transparent transition text-sm"
                                placeholder="Opsional untuk persetujuan, wajib jika ditolak..."></textarea>
                        </div>

                        <button type="submit"
                            class="w-full py-4 bg-maroon text-white font-bold rounded-xl hover:bg-maroon/90 shadow-lg shadow-maroon/20 transition-all flex items-center justify-center">
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            ...
                            <div class="mt-2 text-sm text-blue-700">
                                <p>Setelah disetujui, jadwal ini akan langsung masuk ke daftar <strong>Jadwal Aktif</strong>
                                    dan bisa dilihat oleh Dosen & Mahasiswa.</p>
                            </div>
                        </div>
                    </div>
                </div>
