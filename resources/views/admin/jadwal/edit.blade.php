                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-700 mb-2"><i
                                class="fas fa-book text-gray-400 mr-1"></i>Mata Kuliah *</label>
                        <select name="mata_kuliah_id" x-model="mataKuliahId" @change="fetchDosens()"
                            ...
                            @foreach($mataKuliahs as $mk)
                                <option value="{{ $mk->id }}" {{ old('mata_kuliah_id', $kelasMataKuliah->mata_kuliah_id) == $mk->id ? 'selected' : '' }}>{{ $mk->kode_mk }} -
                                    {{ $mk->nama_mk }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2"><i
                                class="fas fa-users text-gray-400 mr-1"></i>Nama Kelas *</label>
                        <input type="text" name="nama_kelas" value="{{ old('nama_kelas', $kelasMataKuliah->kode_kelas) }}"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-maroon-500 focus:border-transparent transition"
                            placeholder="A, B, C..." required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2"><i
                                class="fas fa-user-tie text-gray-400 mr-1"></i>Dosen *</label>
                        ...
                        <div x-show="!loading && mataKuliahId && dosens.length === 0"
                            class="w-full px-4 py-3 border border-red-300 rounded-lg bg-red-50 text-red-500">
                        ...
                        <template x-if="!loading && dosens.length === 1">
                            <div>
                                ...
                                <div
                                    class="w-full px-4 py-3 border border-green-300 rounded-lg bg-green-50 text-green-700 flex items-center justify-between">
                        ...
                        <select x-show="!loading && dosens.length > 1" name="dosen_id" x-model="selectedDosenId"
                            ...
                            <template x-for="dosen in dosens" :key="dosen.id">
                                <option :value="dosen.id" x-text="dosen.name" :selected="dosen.id == selectedDosenId">
                                </option>
                            </template>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="hari" x-model="hari" @change="checkRoom()"
                            ...
                            @foreach(['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'] as $h)
                                <option value="{{ $h }}" {{ old('hari', $kelasMataKuliah->hari) == $h ? 'selected' : '' }}>
                                    {{ $h }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2"><i
                                class="fas fa-clock text-gray-400 mr-1"></i>Jam Mulai <span
                                class="text-red-500">*</span></label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2"><i
                                class="fas fa-clock text-gray-400 mr-1"></i>Jam Selesai <span
                                class="text-red-500">*</span></label>
                    ...
                    <div class="md:col-span-2" x-show="!roomStatus.available">
                        <div
                            class="bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-lg flex items-start gap-2">
                    ...
                    <div class="md:col-span-2" x-show="checkingRoom">
                        <div
                            class="bg-blue-50 border border-blue-200 text-blue-600 px-4 py-3 rounded-lg flex items-center gap-2">
