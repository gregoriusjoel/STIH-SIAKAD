    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-xl shadow-lg border-t-4 border-maroon">
            <div class="p-6 border-b border-gray-200 bg-maroon text-white rounded-t-xl">
                <h3 class="text-xl font-bold flex items-center"><i class="fas fa-clock mr-3 text-2xl"></i>Tambah Jadwal
                    Perkuliahan</h3>
            </div>
            <form action="{{ route('admin.jadwal.store') }}" method="POST" class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2"><label class="block text-sm font-medium text-gray-700 mb-2"><i
                                class="fas fa-chalkboard-teacher text-gray-400 mr-1"></i>Kelas Mata Kuliah *</label><select
                            name="kelas_mata_kuliah_id"
                            ...
                            required>
                            <option value="">Pilih Kelas</option>@foreach($kelasMatKul as $k)<option value="{{ $k->id }}">
                                {{ $k->mataKuliah->nama_mk }} (Kelas {{ $k->nama_kelas }}) - {{ $k->dosen->user->name }}
                            </option>@endforeach
                        </select></div>
                    ...
                    <div><label class="block text-sm font-medium text-gray-700 mb-2"><i
                                class="fas fa-door-open text-gray-400 mr-1"></i>Ruangan *</label><input type="text"
                            name="ruangan"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-maroon-500 focus:border-transparent transition"
                            placeholder="R.101" required></div>
                    <div><label class="block text-sm font-medium text-gray-700 mb-2"><i
                ...
                <div class="flex justify-end space-x-3 mt-8 pt-6 border-t"><a href="{{ route('admin.jadwal.index') }}"
                        ...
                        class="bg-maroon text-white px-6 py-3 rounded-lg hover:bg-red-900 transition flex items-center shadow-md transform hover:scale-105"><i
