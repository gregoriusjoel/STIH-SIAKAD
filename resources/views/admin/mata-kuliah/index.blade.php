    ...
    <div x-data="mkPage()" x-init="init()">
                    ...
                    @foreach($allSemesters as $sem)
                        <form action="{{ route('admin.mata-kuliah-semester.activate-semester', $sem) }}" method="POST"
                            ...
                            <button type="submit"
                                ...
                                @if($sem->is_active)<i class="fas fa-check-circle text-green-500 w-4"></i>@else<span
                                class="w-4 inline-block"></span>@endif
                                {{ $sem->display_label }}
                            </button>
                        </form>
                    @endforeach
            ...
            <div class="ml-auto flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400">
                @if($selectedSemester)
                    <span>MK Aktif TA: <strong
                            class="text-maroon dark:text-red-400">{{ $activePivots->count() }}</strong></span>
        ...
        <div
            ...
            <a href="{{ route('admin.mata-kuliah.index', ['tab' => 'master', 'semester_id' => $selectedSemester?->id]) }}"
                ...
                <span
                    class="ml-1 px-1.5 py-0.5 rounded-full text-xs {{ $tab === 'master' ? 'bg-maroon text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400' }}">{{ $mataKuliahs->total() }}</span>
            </a>
            <a href="{{ route('admin.mata-kuliah.index', ['tab' => 'ta-aktif', 'semester_id' => $selectedSemester?->id]) }}"
                class="px-5 py-3 text-sm font-semibold border-b-2 transition flex-shrink-0 {{ $tab === 'ta-aktif' ? 'border-maroon text-maroon dark:text-red-400 dark:border-red-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-maroon' }}">
                <i class="fas fa-check-circle mr-1.5"></i> TA Aktif
                <span
                    class="ml-1 px-1.5 py-0.5 rounded-full text-xs {{ $tab === 'ta-aktif' ? 'bg-maroon text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400' }}">{{ $activePivots->count() }}</span>
            </a>
            <a href="{{ route('admin.mata-kuliah.index', ['tab' => 'histori', 'semester_id' => $selectedSemester?->id]) }}"
                ...
                <span
                    class="ml-1 px-1.5 py-0.5 rounded-full text-xs {{ $tab === 'histori' ? 'bg-maroon text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400' }}">{{ $historyPivots->count() }}</span>
            </a>

            {{-- Search & Filter di sebelah kanan tab --}}
            <div class="ml-auto flex items-center gap-2 px-3 py-2">
                @if($tab === 'master')
                    {{-- Master MK: server-side filter via form submit --}}
                    <form method="GET" action="{{ route('admin.mata-kuliah.index') }}" class="flex items-center gap-2"
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="mk-search" name="search" value="{{ $search }}"
                ...
                @else
                    {{-- TA Aktif / Histori: client-side filter --}}
                    <div class="relative">
                        <i class="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs"></i>
            ...
            @if($tab === 'master')
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-maroon text-white">
                            <tr>
                                ...
                                @endphp
                                <th class="px-5 py-4 text-left text-xs font-bold uppercase"><a href="{{ $sortUrl('kode_mk') }}"
                                        class="hover:underline">Kode MK {!! $sortIcon('kode_mk') !!}</a></th>
                                <th class="px-5 py-4 text-left text-xs font-bold uppercase"><a href="{{ $sortUrl('nama_mk') }}"
                                        class="hover:underline"><i class="fas fa-book-open mr-1"></i>Nama Mata Kuliah
                                        {!! $sortIcon('nama_mk') !!}</a></th>
                                <th class="px-5 py-4 text-center text-xs font-bold uppercase"><a href="{{ $sortUrl('sks') }}"
                                        class="hover:underline"><i class="fas fa-calculator mr-1"></i>SKS
                                        {!! $sortIcon('sks') !!}</a></th>
                                <th class="px-5 py-4 text-center text-xs font-bold uppercase"><a href="{{ $sortUrl('jenis') }}"
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700" id="master-tbody">
                            @forelse($mataKuliahs as $mk)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition master-row"
                                            ...
                                            <td class="px-5 py-4">
                                                <span
                                                    class="font-mono text-sm text-maroon dark:text-red-400 font-bold">{{ $mk->kode_mk }}</span>
                                            </td>
                                            <td class="px-5 py-4">
                                                <div class="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <div class="text-sm font-semibold text-gray-900 dark:text-gray-100">
                                                            {{ $mk->nama_mk }}</div>
                                                        <div class="text-xs text-gray-500 dark:text-gray-400">Semester {{ $mk->semester }}
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                    ...
                                    : 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300') }}">
                                                    ...
                                                    <div>
                                                        <div class="text-sm font-medium text-gray-900 dark:text-gray-100">
                                                            {{ $mk->prodi->nama_prodi }}</div>
                                                        <div class="text-xs text-gray-500 dark:text-gray-400">
                                                            {{ $mk->fakultas->nama_fakultas }}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-5 py-4 text-center">
                                        ...
                                        <div id="modal-mk-{{ $mk->id }}"
                                            ...
                                            <div
                                                ...
                                                <div class="flex items-center justify-between px-6 py-4 bg-maroon text-white rounded-t-xl">
                                                    ...
                                                    <button
                                                        onclick="document.getElementById('modal-mk-{{ $mk->id }}').classList.add('hidden')"
                ...
                @if($mataKuliahs->hasPages())
                    <div class="bg-gray-50 dark:bg-gray-800 px-6 py-4 border-t border-gray-200 dark:border-gray-700">
                        {{ $mataKuliahs->links(null, ['path' => route('admin.mata-kuliah.index'), 'query' => array_filter(['tab' => 'master', 'semester_id' => $selectedSemester?->id, 'search' => $search, 'semester_filter' => $semesterFilter, 'sort' => $sort, 'sort_dir' => $sortDir], fn($v) => $v !== '' && $v !== null)]) }}
                    </div>
                @endif
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700" id="active-tbody">
                                @foreach($activePivots as $i => $pivot)
                                    ...
                                    <tr class="hover:bg-red-50 dark:hover:bg-red-900/10 transition active-row"
                                        ...
                                        data-semester="{{ $mk->semester }}" data-sks="{{ $mk->sks }}">
                                        <td class="px-4 py-3">
                                            <input type="checkbox" value="{{ $mk->id }}"
                                        ...
                                        <td class="px-4 py-3">
                                            <span
                                                class="font-mono text-xs font-bold text-maroon dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2 py-0.5 rounded">{{ $mk->kode_mk }}</span>
                                        </td>
                                        <td class="px-4 py-3">
                                        ...
                                        <td class="px-4 py-3 text-center">
                                            <span
                                                class="px-2 py-0.5 text-xs font-bold rounded-full bg-red-100 dark:bg-red-900/30 text-maroon dark:text-red-300">{{ $mk->sks }}
                                                SKS</span>
                                        </td>
                                        <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $mk->prodi?->nama_prodi }}
                                        </td>
                                        <td class="px-4 py-3">
                                            <span
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700" id="history-tbody">
                                @foreach($historyPivots as $i => $pivot)
                                    ...
                                    <tr class="hover:bg-red-50 dark:hover:bg-red-900/10 transition history-row {{ $pivot->status === 'archived' ? 'opacity-60' : '' }}"
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ $mk->nama_mk }}</div>
                                            <div class="text-xs text-gray-500 dark:text-gray-400">{{ $mk->sks }} SKS &bull; Semester
                                                {{ $mk->semester }}</div>
                                        </td>
                                        <td class="px-4 py-3 text-center">
                                            <span
                                                class="px-2 py-0.5 text-xs font-bold rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400">{{ $mk->sks }}
                                                SKS</span>
                                        </td>
                                        <td class="px-4 py-3 text-center">
                                            <span
                                        ...
                                        <td class="px-4 py-3 text-xs text-gray-500 dark:text-gray-400">
                                            {{ $pivot->sourceSemester?->display_label ?? '—' }}</td>
                                        <td class="px-4 py-3 text-center text-xs text-gray-500 dark:text-gray-400">
        ...
        <div x-show="attachOpen" x-cloak
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col"
                ...
                <form method="POST" action="{{ route('admin.mata-kuliah-semester.attach') }}"
                    ...
                    <div class="p-5 flex-1 overflow-y-auto">
                        <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
                            ...
                            <strong>{{ $selectedSemester?->display_label }}</strong>.
                        </p>
                            ...
                            @forelse($unattachedMK as $mk)
                                <label
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="text-sm font-semibold text-gray-800 dark:text-gray-100">{{ $mk->nama_mk }}
                                        </div>
                                        <div class="text-xs text-gray-500 dark:text-gray-400">
                    ...
                    <div class="p-5 flex-1 overflow-y-auto">
                        <div class="mb-4">
                            ...
                            <select name="source_semester_id"
                                ...
                                @foreach($allSemesters as $sem)
                                    <option value="{{ $sem->id }}" {{ $sem->id === $selectedSemester?->id ? 'selected' : '' }}>
                                        {{ $sem->display_label }}</option>
                                @endforeach
                            </select>
                        </div>
                                ...
                                <label
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="text-sm font-semibold text-gray-800 dark:text-gray-100">
                                            {{ $pivot->mataKuliah->nama_mk }}</div>
                                        <div class="text-xs text-gray-500 dark:text-gray-400">
                                            {{ $pivot->mataKuliah->kode_mk }} &bull; {{ $pivot->mataKuliah->sks }} SKS
                                        </div>
                                    </div>
                                    <span
                                        class="px-2 py-0.5 text-xs rounded-full bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400">{{ $pivot->status }}</span>
                                </label>
                            @empty
                                <div class="text-center py-8 text-gray-400 dark:text-gray-500">
                    ...
                    <div
                        ...
                        <div class="flex flex-wrap gap-2 text-xs">
                            @foreach(['kode_mk', 'nama_matkul', 'sks', 'semester'] as $col)
                                <span
                                    class="px-2 py-1 rounded-full bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 font-semibold">{{ $col }}
                                    *</span>
                            @endforeach
                            @foreach(['kode_id', 'jenis', 'praktikum', 'prodi_id', 'fakultas_id', 'deskripsi'] as $col)
                                <span
                                    class="px-2 py-1 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400">{{ $col }}</span>
                            @endforeach
                        </div>
